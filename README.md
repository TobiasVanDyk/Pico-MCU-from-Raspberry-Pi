...
A Touch Macropad with 72 + 99 macrokeys, each of which can be chained 10x, and more than 30 different pages and layouts for actions such as Media-keys (Volume Mute Play-Stop Next-Previous Bass-Treble), inserting special and math-symbols, a Restart-Shutdown Timer and Time(Clock) Config, Timer-and-Time-executed repeat-and-oneshot macros, and a macro composition and linking editor etc. The Touch LCD is used most conveniently when placed upright rather than flat, and using your thumb-tip to press the 5 option pads. Place two fingers on top of the LCD to stabilise it when using your thumb to press the keys.
...
run dialog and then open notepad Add macro m 03 to the link list - press [Num] until it shows 

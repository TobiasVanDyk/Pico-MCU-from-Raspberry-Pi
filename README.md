...
Where available the SDCard slot can also be used for saving and reading files. Keys \[S1\] to \[S24\] Send SDCard files as keyboard characters to the PC - the size of the files/strings are only limited by the SDCard size. Change between three SDCard files set of 24 files each with \*sd\*n where n=1,2,3. 

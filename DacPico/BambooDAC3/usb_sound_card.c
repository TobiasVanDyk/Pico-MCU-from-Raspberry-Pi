...
#define LED6 6 // Use GPIO 6
#define LED7 7 // Use GPIO 7
...
static void _as_audio_packet(struct usb_endpoint *ep) {
    ...
    // If muted, zero the audio data before sending it to the I2S pipeline.
    // This keeps host timing/feedback intact while producing silence on output.
    if (audio_state.mute && usb_buffer->data_len > 0) {
        memset(usb_buffer->data, 0, usb_buffer->data_len);
    }
...
static void _audio_reconfigure() {
    switch (audio_state.freq) {
        case 44100: // gpio_put(LED6, 0);  gpio_put(LED7, 1); 
        case 48000: // gpio_put(LED6, 1);  gpio_put(LED7, 0); 
        case 88200: 
        case 96000: // gpio_put(LED6, 1);  gpio_put(LED7, 1); 
//        case 176400:
//        case 192000:
            break;
        default:
...
static bool as_set_alternate(struct usb_interface *interface, uint alt) {
    ...
    switch (alt) {
        case 3:
            ...
            gpio_put(LED6, 0);  gpio_put(LED7, 1); 
            break;
        case 2:
            audio_state.resolution = 24;
            gpio_put(LED6, 1);  gpio_put(LED7, 0); 
            break;
        case 1:
            audio_state.resolution = 16;
            gpio_put(LED6, 1);  gpio_put(LED7, 1); 
            break;
        case 0:
        default:
            break;
...
int main(void) {
    ...
    gpio_init(LED6); gpio_init(LED7);
    gpio_set_dir(LED6, GPIO_OUT); gpio_set_dir(LED7, GPIO_OUT);
    gpio_put(LED6, 0);  gpio_put(LED7, 0); 

...
char NameStr1[30] = { "" };        // Used in: ListMacro() RemoveMacro() DoLinkStr() DoKeyMST() DoKey16() - NameStr1
...
{ char *ChrPtr;
  ...
  if (Option) {
      if (LittleFS.exists("Config1"))           ReadConfig1();          else WriteConfig1(0);           // Read Config1 else write and use defauklt values
...
{ uint8_t keycode[6]; // simultaneous keys pressed in here
  ...
  if (MST2==3) { if (!sdCard) f = LittleFS.open(NameStr, "w");      // Only save to file axx if option target is a
                 if (sdCard)  f = SDFS.open(NameStr, "w");            // Only save to file axx if option target is a

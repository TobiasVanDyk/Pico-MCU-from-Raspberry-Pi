...
//                       L0n L1n 0: Do not restore 1: Restore LayerAxD on completion
//
...
{ uint8_t keycode[6] = { 0 };      // simultaneous keys pressed in here
  ...
  while (j<NameStrLen-1) 
        {for (i=0; i<3; i++) { NameStr[i] = inputString[i+j]; } 
         ...
         if (c=='l'||c=='L') { if (b==1) LayerAxDSave = LayerAxD;                                                // Restore after completion
                               if (a==3) LayerAxD = !LayerAxD; if (a==1||a==2) LayerAxD = a-1;                   // L01Xnn Flash L02Xnn SSDCard
                               if (b==0) LayerAxDSave = LayerAxD;                                                // Do not restore after completion 

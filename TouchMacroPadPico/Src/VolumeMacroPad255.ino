...
//
// Adapted by Tobias van Dyk August 2022 - February 2025 - for ILI9488 480x320 LCD and Adafruit TinyUSB stack
...
//           nKeys 1  nChar 'n'  nKeysPage 8  nKeysCharSet=10charslist
// Unused: MuteDisable VolDisable LayerAxD MediaKeys XFiles Brightness
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const static  byte Config1Size = 30;
byte Config1[Config1Size]                   = {1,0,1,1,0,0,0,1,'n',8,'n','o','p','q','r','s','t','m','a','k',0,0,0,0,0,0,0,0,0};
const static byte Config1Reset[Config1Size] = {1,0,1,1,0,0,0,1,'n',8,'n','o','p','q','r','s','t','m','a','k',0,0,0,0,0,0,0,0,0};
...
"Select nKeys Character use Pad (o)",   "Select SDCard FileSet use Pad (o)",     "File List SDCard + Flash use Pad (o)" };
...
const int nKeySize = 30;         // Indirection i.e. n01-n96 keys only holds [/folder][/]filename[.ext] of target file
...
const static int StarCodesMax = 80; // StarCodes Count 16+16+16+16+16 StarNum = 0-79
...
{ "*am*", "*as*", "*at*", "*bb*", "*bl*", "*br*", "*ca*", "*cm*", "*ct*", "*db*", "*de*", "*df*", "*e0*", "*e1*", "*e2*", "*e3*", 
  ...
  "*lf*", "*m1*", "*m2*", "*mt*", "*mT*", "*nt*", "*nT*", "*os*", "*ot*", "*oT*", "*po*", "*r0*", "*r1*", "*r2*", "*r3*", "*rn*", 
  "*ro*", "*rt*", "*rT*", "*sa*", "*sd*", "*se*", "*sm*", "*ss*", "*st*", "*ta*", "*tb*", "*tp*", "*tt*", "*tw*", "*ua*", "*ul*", 
  "*up*", "*x0*", "*x1*", "*x2*", "*x3*", "*x4*", "*x5*", "*x6*", "*x7*", "*x8*", "*x9*", "*0R*", "*09*", "*0n*", "*0p*", "*0s*"  };
...
{ 1,      1,      1,      2,      36,     5,      6,      7,      8,      3,      9,      17,     10,     10,     10,     10,     
  ...
  42,     18,     19,     20,     20,     21,     21,     22,     23,     23,     25,     37,     26,     40,     41,     49, 
  27,     24,     24,     28,     29,     30,     28,     28,     28,     31,     4,      31,     31,     31,     33,     32,     
  43,     35,     35,     35,     35,     35,     35,     35,     35,     35,     35,     34,     45,     46,     47,     48  };
...
{ char *ChrPtr;
  ...
  File f;
  uint16_t StrLen = 0;
  char nFile[30] = "1234567890";             // Name of current nKey content i.e. [path]filename  
  char MacroChar[11] = {"stmakSTMAK"};       // These are handled as macros/strings not file paths
  
  int c = Button*(Numkeys123+1);             // nKeys n01-n96 
  
  if (LayerAxD)  f = SDFS.open(NKeysX[Button], "r"); 
  if (!LayerAxD) f = LittleFS.open(NKeysX[Button], "r");
  StrLen = f.size();
  f.readBytes(nFile, StrLen);
  f.close(); 
  nFile[StrLen] = 0;
  if (StrLen<3) { status("No valid filename inside nKeys files found"); return; }  // At least one char filename

  Serial.println(StrLen);
  Serial.println(NKeysX[Button]);
  Serial.println(nFile);

  ////////////////////////////////////////////////////////// Test is m,s,t,a,k or M,S,T,A,K then handle as macro/text file
  ...
  ///////////////////////////////////////////////////////// File is a linkefile
  if ( nFile[StrLen-4]=='L' && nFile[StrLen-1]=='k' )    // Last part of filename is Lxxk or Link
     { isLink = true;                 
       LinkOk = false;
       n = 0;
  ...
  if (MacroBuffSize==0) { status("No valid content from filename inside files n01-n96"); return; }

  //Serial.println(MacroBuffSize);
     ...
     { PadKeys = true; 
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (OptNum>0) { if (OptNum==1) { MacroUL = !MacroUL; SwitchMacroUL(1); ConfigButtons(1); return; }  
                                         ...
                                         if (OptNum==7) { nKeysCurr[0] = nChar = nKeysCharSet[nCharNum]; nCharNum++; if (nCharNum>9) nCharNum=0;  
...
{ char *ChrPtr;
  ...
  Serial.println("Version: VolumeMacro255 Tobias van Dyk February 2025 License GPL3");
  Serial.println("Hardware: Waveshare ILI9486 RPi Resistive TouchLCD 3.5inch Type C"); 
...
// If n-Keys action here is to display content of n01-n96 files - does not execute the content
...
{ uint8_t keycode[6]; // simultaneous keys pressed in here
  ...
  char status2[]  = {"mstaknMSTAKN"} ;          // MST2 0 1 2 3 4 5 -> m s t a k n
  char NameStr[4] = {"xnn"}  ;  
  File f;
  ...
  if (macroA) { DoMSTAName(MST2A, 3);             strcpy(NameStr, MSTAName); 
        ...
        else  { DoMSTAName(Option2, 3+(MST2==4)+(MST2==5)); strcpy(NameStr, MSTAName); 
...
{ char KeyBrd5status[16] =   {"Macro nn -> Xnn"};
  ...
  MST = MacroBuffSize = 0;
  a = KeyBrdByte[6];    // M S T A K N = 77 83 84 65 75 78 = 0 1 2 3 4 5
  b = (a==65||a==97)*3 + (a==75||a==107)*4  + (a==77||a==109)*0 + (a==83||a==115)*1 + (a==84||a==116)*2 + (a==78||a==110)*5; // aA kK mM sS tT nN
...
{ 
  ...
  if (b==0x4E) MST = 5; // 0x4E=77  N Keys 1-96
  return MST;
}
///////////////////////////////////////////////////////////////////// 
void DoSourceMacro()    
...
{ byte a, b;
  ...
  if (KeyBrdByteNum==0) {if (NumMode==1) { Option1++; 
                             ...
                             if (MST1>2)  if (Option1>98) Option1=0; // A K = 01 - 99
                             if (MST1>4)  if (Option1>95) Option1=0; // N = 01 - 96 (12 x 8 pages max)
                             status("Source Macro Number");  }  
                         if (NumMode==2) { Option2++; 
                             if (MST2<3)  if (Option2>23) Option2=0;
                             if (MST2>2)  if (Option2>98) Option2=0; 
                             if (MST2>4)  if (Option2>95) Option2=0; // N

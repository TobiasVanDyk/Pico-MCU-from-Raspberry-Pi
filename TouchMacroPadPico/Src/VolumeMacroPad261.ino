...
//#define cSt const static // need space
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Used:   KeySkip 1  CheckSerial 0  KeyHeldEnable 1  BLOnOff 1  Rotate180 0  KeyFontBold 0  ResetOnceEnable 0
//           nKeys 1  nChar 'n'  nKeysPage 8  nKeysCharSet=10charslist CRLF 0 crlf1 0x0D crlf2 0x0A
// Unused: MuteDisable VolDisable LayerAxD MediaKeys XFiles Brightness
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const static  byte Config1Size = 30;
byte Config1[Config1Size]                   = {1,0,1,1,0,0,0,1,'n',8,'n','o','p','q','r','s','t','m','a','k',0,0x0D,0x0A,0,0,0,0,0,0};
const static byte Config1Reset[Config1Size] = {1,0,1,1,0,0,0,1,'n',8,'n','o','p','q','r','s','t','m','a','k',0,0x0D,0x0A,0,0,0,0,0,0};
...
// Waveshare Pico ILI9488 3.5" LCD needs different dimmed LCD set
const static uint8_t dbArr[10] = {   0,    25,  29,   34,   39,   44,   51,   64,  102,  128 };
const static uint8_t bbArr[10] = {   0,    29,  34,   39,   44,   51,   64,  102,  128,  166 };
const static char DBArr[10][3] = { " 0", " 8", "10", "12", "15", "17", "20", "25", "40", "50"};
const static char BBArr[10][4] = {"100", " 10","12", "15", "17", "20", "25", "40", "50", "65"};
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//uint16_t calData[5] = {277, 3636, 279, 3501, 5};  // rotate = 0
//uint16_t calData[5] = {297, 3572, 288, 3476, 3};  // rotate = 180     
uint16_t calData[5];                              
uint8_t  calDataOK = 0;
...
uint8_t  keydelay0 = 25;        // keyPress delay
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const int nKeySize = 200;            // Indirection i.e. n01-n96 keys only holds [/folder][/]filename[.ext] of target file
...
bool nKeys = true;                   // Numpad Pages or Macro keys n01 - n96 change char n with *0n*char 
int  nKeysPage = 8;                  // Number of nKeys and nUmPad pages change with *0p*pages where pages = 1-8
const int nKeysPageMax = 8;          // Maximum number of nKeys and nUmPad pages change with *0p*pages where pages = 1-8
...
const static char NumkeysLabel[nKeysPageMax][12][4] = 
...
const static int StarCodesMax = 82; // StarCodes Count 16+16+16+16+16+2 StarNum = 0-81
...
{ "*am*", "*as*", "*at*", "*bb*", "*bl*", "*br*", "*ca*", "*cm*", "*cr*", "*ct*", "*cx*", "*db*", "*de*", "*df*", "*e0*", "*e1*", 
  ...
  "*ua*", "*ul*", "*up*", "*x0*", "*x1*", "*x2*", "*x3*", "*x4*", "*x5*", "*x6*", "*x7*", "*x8*", "*x9*", "*0R*", "*09*", "*0n*", 
  "*0p*", "*0s*"  };
...
{ 1,      1,      1,      2,      36,     5,      6,      7,      50,     8,      51,     3,      9,      17,     10,     10,     
  ...
  33,     32,     43,     35,     35,     35,     35,     35,     35,     35,     35,     35,     35,     34,     45,     46,     
  47,     48  };
...
{ SPI1.setSCK(pinSdClk);
  ...
  SDFSConfig c2;      // See C:\Users\Tobias\AppData\Local\Arduino15\packages\rp2040\hardware\rp2040\3.9.2\libraries\SDFS\src\SDFS.h
  ...
  if (calData[4]==3 && Rotate180==0) Rotate180=1;       // Correct wrong combination of Rotate180 and calData[4]          
...
{ bool LinkFound = false;
  ...
  if (LayerAxD) { if (SDFS.remove(MSTLinkName));     LinkFound = true; }
...
{ if (Restore) { status(" ") ;                                                   }                            
  ...
  if (NumKeys) status("Press Option Pad (o) for next page"); else status(" ");
}
...
{ uint8_t keycode[6] = { 0 };   // simultaneous keys pressed in here
  ...
  if (MacroBuff[0]<0x80) { n=0; while (MacroBuff[n]!=0 && MacroBuffSize>n) { usb_hid.keyboardPress(HIDKbrd, MacroBuff[n]); delay(keydelay0);
                                                                             usb_hid.keyboardRelease(HIDKbrd);             delay(keydelay0); 
...
/////////////////////////////////////////////////////////////////////////////////////////////////
// The 96 n01-n96 keys (n changeable to 0-9, aA-zZ) are default white label on Cyan background
/////////////////////////////////////////////////////////////////////////////////////////////////
{ char *ChrPtr;
  uint8_t keycode[6] = { 0 };      // simultaneous keys pressed in here
  ...
  int c = Button*(Numkeys123+1);                   // nKeys x01-x96 
  BPtr = MacroBuff; 
  MacroBuffSize = 0;
  MacroBuff[0] = 0x00;
  ...
  for (n=0; n<10; n++) if (a==MacroChar[n]) { isMacro = true; if (MacroKeys(c, 3)) return; }   // mstak-MSTAK files is macro/string  
  
  if (LayerAxD)  f = SDFS.open(NKeysX[Button], "r"); else f = LittleFS.open(NKeysX[Button], "r");
  nStrLen = f.size();
  if (nStrLen<nKeySize) { f.readBytes(nFile, nStrLen); f.close(); nFile[nStrLen] = 0; }         // Valid Filename     
  if (isMacro) strcpy(nFile, NKeysX[Button]);                                                   // Handle MSTAK large file
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////// File is a linkefile
  if ( nFile[nStrLen-4]=='L' && nFile[nStrLen-1]=='k' )    // Last part of filename is Lxxk or Link
     { n = 0;  
       LayerAxDSave = LayerAxD;
  ...
  if ((Button>11)&&(Layout==2)) 
     { if (OptNum==1 && Button!=16) OptNum=0; // Switch off Pad[o] nKeys char select if Pad[n] nKeys is pressed
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (OptNum>0) { if (OptNum==1) { 
                          nKeysCurr[0] = nChar = nKeysCharSet[nCharNum]; nCharNum++; if (nCharNum>9) nCharNum=0;  
                         ...
                         if (NumKeys) { if (Numkeys123<nKeysPage-1) Numkeys123++ ; else Numkeys123 = 0; 
  ...
  if (Layout==2) {  
    ...
    case 1: // ARROW UP ////////////////////////////////////////////////
      ...
      if (NumKeys) {if (nKeys) DoNKeys(Button);
                         else {usb_hid.keyboardPress(HIDKbrd, NumkeysX[Button][0]); delay(keydelay);
                               usb_hid.keyboardRelease(HIDKbrd);                 }  break;        }    
    ...
    case 4: // ARROW LEFT //////////////////////////////////////////////
      ...
      if (NumKeys) { if (nKeys) DoNKeys(Button);
                          else {usb_hid.keyboardPress(HIDKbrd, '\r'); delay(keydelay);
                                usb_hid.keyboardRelease(HIDKbrd);  }  break;        }      
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (NumKeys) { if (nKeys)  { for (m=0; m<12; m++) {strcpy(keyLabel[m], NKeysX[m]);}
                               for (m=0; m<12; m++) {keyColor[m] = dCyan;} }
  ...
  for (row = 0 + rowcount; row < rows; row++) {   // 3 rows 0,1,2 => b = 0+(0*4)=0 1+(0*4)=1 ... 0+(1*4)=4 ....
   ...
   DoPadKeys:
   ////////////////////////////////////////////////////
   // 5 Small Config Buttons between 1 st and 3rd row 
   //////////////////////////////////////////////////// 
   b = Layout - 1;                     // reset b to 0 - 3 
   if (nKeys) padLabel[8][0] = nChar; else padLabel[8][0] = 'N';
   for (row = 0; row < 5; row++) 
...
////////////////////////////////////////////////
// Number Pad Page or Macrokeys n1-n96 Select
////////////////////////////////////////////////
void NumKeysChange()
{ int a, b, c;
  c = (Numkeys123)*12;    // c = 0, 12, 24, 36, 48, 60, 72, 84, 96
  for (int m=0; m<12; m++) { if (nKeys) { a = 48 + (c+m+1)/10; b = 48 + (c+m+1)%10; 
                                          NKeysX[m][0] = nChar; NKeysX[m][1] = a; NKeysX[m][2] = b; NKeysX[m][3] = 0x00; }
                                  else  { strcpy(NumkeysX[m], NumkeysLabel[Numkeys123][m]);  }  }
  ConfigButtons(5); // Only update 5 Pad keys                                        
}
///////////////////
void ReadBank123()
///////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ReadConfig1()
// Used:   KeySkip 1  CheckSerial 0 KeyHeldEnable 1 BLOnOff 1  Rotate180 0 KeyFontBold 0 ResetOnceEnable 0 nKeys 1 
//         nChar 'n'  nKeysPage 8 
// Unused: MuteDisable VolDisable LayerAxD MediaKeys XFiles Brightness
// byte Config1[20] = {1,0,1,1,0,0,0};
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
{ File f1 = LittleFS.open("Config1", "r"); 
  ...
  BLOnOff = Config1[3];   
  Rotate180 = Config1[4];                                    // Must check when rotate=off but calibrate-data is for rotate=180  
  ...
  for (int n=1; n<11; n++) nKeysCharSet[n-1] = Config1[9+n]; // if (nKeysCharSet[0]<0x30) reset all 10 to default
...
{if (Option==1) { Config1[0] = KeySkip;
                  ...
                  Config1[8] = nChar;
                  Config1[9] = nKeysPage; 
                  for (int n=1; n<11; n++) Config1[9+n] = nKeysCharSet[n-1];
                  Config1[20] = CRLF; 
                  Config1[21] = crlf1; 
                  Config1[22] = crlf2;               
                  }
...
{ char *ChrPtr;
  ...
  if (Option) {
      ...
      if (LittleFS.exists("Config1"))           ReadConfig1();          else WriteConfig1(0);           // Read Config1 else write and use default values
      if (crlf1==0 && crlf2==0) { crlf1 = 0x0D; crlf2 = 0x0A;  }                                        // Reset to default
      ...
      if (LittleFS.exists("iListOn"))           iList         = true;   else iList           = false;   // iList On or Off 
      ...
      if (LittleFS.exists("LayerAD"))           { ChrPtr = LayerADFile; DoFileStrings(0, "LayerAD", ChrPtr, 0); LayerAD  = LayerADFile[0] - 65; }       
      if (LittleFS.exists("DeleteBackspace"))   { ChrPtr = BsDNumFile;  DoFileStrings(0, "DeleteBackspace", ChrPtr, 0); BsDNum  = BsDNumFile[0] - 65; }  
      if (LittleFS.exists("ReturnKey"))         { ChrPtr = RetNumFile;  DoFileStrings(0, "ReturnKey", ChrPtr, 0);       RetNum  = RetNumFile[0] - 65; }  
...
{ char *ChrPtr;
  ...
  if (Action>0) 
     { if (SaveLayout[0]>0) { SaveLayoutConfig(); }         //  else LittleFS.remove("Layout"); 
       if (Action==4) { Savex1x6(); SaveX1X6 = false; 
                        if (XFiles) { XFileNum[0] = '1'; ChrPtr = XFileNum;  DoFileStrings(true, "XFiles", ChrPtr, 0); } // x1-x6 top row keys
  ...
  if (LayerADChange) { LayerADFile[0] = LayerAD + 65; ChrPtr = LayerADFile; DoFileStrings(true, "LayerAD",  ChrPtr, 0); LayerADChange = false;
                       if (LayerAxD) { File f = LittleFS.open("LayerAxD", "w"); if (f) {f.print(LayerAxD); f.close(); } } 
                                  else LittleFS.remove("LayerAxD"); }

  if (SaveVar) { BsDNumFile[0] = BsDNum + 65; ChrPtr = BsDNumFile; DoFileStrings(true, "DeleteBackspace",  ChrPtr, 0);     
                 RetNumFile[0] = RetNum + 65; ChrPtr = RetNumFile; DoFileStrings(true, "ReturnKey",  ChrPtr, 0);  SaveVar = false; }
  
  if (VolDisable) {if (!LittleFS.exists("VolDisable")) {char VolD[2]   = "1"; ChrPtr = VolD;  DoFileStrings(true, "VolDisable", ChrPtr, 0);} }
      else LittleFS.remove("VolDisable");   // No repeated write here
      
  if (!MuteDisable) {if (!LittleFS.exists("MuteDisable")) {char MuteD[2]  = "1"; ChrPtr = MuteD; DoFileStrings(true, "MuteDisable",  ChrPtr, 0);} }
       else LittleFS.remove("MuteDisable"); 

  if (Media) {if (!LittleFS.exists("MediaKeys")) {char MediaD[2]  = "1"; ChrPtr = MediaD; DoFileStrings(true, "MediaKeys",  ChrPtr, 0); } }
      else LittleFS.remove("MediaKeys");

  if (OptionOS[0]<3)   { if (SaveOptionOS) SaveOS(); SaveOptionOS = false; }           //  else LittleFS.remove("OptionOS"); 
  
  Serial.println("Version: VolumeMacro260 Tobias van Dyk March 2025 License GPL3");
  ...
  Serial.print(CRLF); Serial.print(" "); Serial.print(crlf1, HEX); Serial.print(" "); Serial.println(crlf2, HEX);
...
{ unsigned long T, z;  // t = timeclock struct
  ...
  switch(c)
      ...
      {  for (n=1;n<12;n++) RecBytes[n] = KeyBrdByte[n+3];
       ...
       { if (KeyBrdByteNum>5) b = b*10 + KeyBrdByte[5]-48;      // *xn*bc b values entered are b = 0, 1-54, n values are 0, 1-9
         n = k2-48; if (n==0) b = 0;                            // n = 0, 1 - 6 or 7,8 for [Del][Ret] or 9 for list 8 keys
         if (KeyBrdByteNum<4&&n>0) { status("Add 1-54 or 0 clear"); delay(1500); break; }    // *x0*0 = clear all but *xn*m n>0 requires m
       ...
       { if (k2==0x78) { iList = !iList; if (iList) { status("Instruction List ON"); File f = LittleFS.open("iListOn", "w"); // *ix*
                                                      if (f) {f.print(iList); f.close(); } } 
                                               else { status("Instruction List OFF"); LittleFS.remove("iListOn");} StarOk = true; break; }       
       ...
       { nKeys = !nKeys; Config1[7] = nKeys; WriteConfig1(0); 
         if (nKeys) status("n-Keys n01-n96 On"); else status ("NumPad On"); NumKeysChange(); StarOk = true; break; }
         case 46: ////////////////////// KeyBrdByte[1]==0x30&&KeyBrdByte[2]==0x6e *0n*char = 1-9 or aA - zZ Select nKeys Char 
       { if (KeyBrdByteNum>5) break; // Single digits or char only after *0n* no filename validity checks
         if (KeyBrdByteNum==5) { status("nKeys character changed"); Config1[8] = nChar = KeyBrdByte[4]; 
         WriteConfig1(1); NumKeysChange(); StarOk = true; break; } }
         case 47: ////////////////////// KeyBrdByte[1]==0x30&&KeyBrdByte[2]==0x70 *0p*pages = 1-8 Select pages number 
       { if (KeyBrdByteNum>5) break; // 1-8 only after *0p* 
         if (KeyBrdByteNum==5 && (b>0&&b<9)) { status("1-8 Pages nKeys/NumPad changed");
                                               Config1[9] = nKeysPage = b; WriteConfig1(1); StarOk = true; } // b = KeyBrdByte[4] - 48; 
         break; }
         case 48: ////////////////////// KeyBrdByte[1]==0x30&&KeyBrdByte[2]==0x73 *0s*10char = set of 10 char to choose from 
       { if (KeyBrdByteNum>5) { status("nKeys set of 10 characters changed");
         for (n=0; n<10; n++) nKeysCharSet[n] = KeyBrdByte[4+n]; WriteConfig1(1); StarOk = true; break; } } 
         case 49: ////////////////////// KeyBrdByte[1]==0x72&&KeyBrdByte[2]==0x6E *rn*newname=oldname  
       { if (KeyBrdByteNum<7) { status("Use *rn*oldname=newname name is file or /folder"); StarOk = true; break; }
         n = 0; while (n<=KeyBrdByteNum-4) { KeyBrdByte[n] = KeyBrdByte[n+4]; n++; } KeyBrdByteNum = KeyBrdByteNum - 4;        
       ...
       { if (KeyBrdByteNum==5 && b<4) { CRLF = Config1[20] = b; WriteConfig1(1);
       ...
       { if (KeyBrdByteNum==6) { crlf1 = Config1[21] = KeyBrdByte[4]; crlf2 = Config1[22] = KeyBrdByte[5]; status("Text filter changed"); StarOk = true; }
         if (KeyBrdByteNum==5 && b==0) { crlf1 = Config1[21] = 0x0D;  crlf2 = Config1[22] = 0x0A;          status("Text filter CR LF");   StarOk = true; } 
...
{ //                  012345678901
  ...
  macroMSTA[5] = macroMSTA[11] = nChar;
  n++;  
...
{ bool RemOK = false;
  ...
  if (sdCard)  RemOK  = SDFS.remove(NameStr1); 
  if (!sdCard) RemOK  = LittleFS.remove(NameStr1);
...
{ uint8_t keycode[6] = { 0 };      // simultaneous keys pressed in here
  ...
  if (MacroBuff[0]<0x80) { n = 0; while (MacroBuff[n]!=0 && MacroBuffSize>n) {usb_hid.keyboardPress(HIDKbrd, MacroBuff[n]);  delay(keydelay2);
                                                                              usb_hid.keyboardRelease(HIDKbrd);              delay(keydelay2); 
...
{ byte a, b;
  ...
  if (KeyBrdByteNum==0) {if (NumMode==1) { Option1++; 
                             ...
                             if (MST1==5)          if (Option1>95) Option1=0; // N = 01 - 96 (12 x 8 pages max)
                             status("Source Macro Number");  }  
                         if (NumMode==2) { Option2++; 
                             if (MST2<3)            if (Option2>23) Option2=0;
                             if (MST1==3||MST1==4)  if (Option2>98) Option2=0; 
                             if (MST2==5)           if (Option2>95) Option2=0; // N

...
//
// Adapted by Tobias van Dyk August 2022 - June 2024 - for ILI9486 480x320 LCD and Adafruit TinyUSB stack
...
#include <SPI.h>
#include <TFT_eSPI.h>
#include <LittleFS.h>    // Case sensitive Up to 255 char filenames
#include <FS.h>
#include <SD.h>          // Case preserving not case sensitive Up to 30(?) char filenames
...
char OSChr [4][2]    = {          "w",          "L",    "R",              "A"           };   // "W" too large for space
...
const static char VarStatus[9][40] = {"",                                                                                                
"[ Delete ] Options press Pad [o]",       "[ Return ] Options press Pad [o]",                                                     // Layout 1,2,3,4
"[ Cut ] L1-Key1 Options press Pad [o]",  "[ Copy ] L1-Key2 Options press Pad [o]",  "[ Paste ] L1-Key3 Options press Pad [o]",   // Layout 1 
"[ cX ] L2 L3 Key1 Options press Pad [o]","[ cC ] L2 L3 Key2 options press Pad [o]", "[ cV ] L2 L3 Key3 Options press Pad [o]" }; // Layout 3,4
const static char OptStatus[9][42] = {"", 
"Macro U/L Case Filenames use Pad [o]", "Vol-Mute long-press On/Off use Pad [o]", "L1 - L4 Startup Select use Pad [o]", 
"[ M ] Keys MacroBank 1-5 use Pad [o]", "[ S ] Keys MacroBank 1-5 use Pad [o]",   "[ T ] Keys MacroBank 1-5 use Pad [o]",
"Select SDCard FileSet use Pad [o]",    "File List SDCard + Flash press Pad [0]" };
...
const static int StarCodesMax = 71; // StarCodes Count 16+16+16+16+7 StarNum = 0-70
const static char StarCode[StarCodesMax][5] =    
{ "*am*", "*as*", "*at*", "*bb*", "*bl*", "*br*", "*ca*", "*cm*", "*ct*", "*db*", "*de*", "*df*", "*e0*", "*e1*", "*e2*", "*e3*", 
  "*e4*", "*fa*", "*fc*", "*fm*", "*fo*", "*fs*", "*ft*", "*kb*", "*ke*", "*kr*", "*ks*", "*ld*", "*lf*", "*m1*", "*m2*", "*mt*", 
  "*mT*", "*nt*", "*nT*", "*os*", "*ot*", "*oT*", "*po*", "*r0*", "*r1*", "*r2*", "*r3*", "*ro*", "*rt*", "*rT*", "*sa*", "*sd*", 
  "*se*", "*sm*", "*ss*", "*st*", "*ta*", "*tb*", "*tp*", "*tt*", "*tw*", "*ua*", "*ul*", "*up*", "*x0*", "*x1*", "*x2*", "*x3*", 
  "*x4*", "*x5*", "*x6*", "*x7*", "*x8*", "*x9*", "*0R*"  };

const static byte StarCodeType[StarCodesMax] =    
{ 1,      1,      1,      2,      36,     5,      6,      7,      8,      3,      9,      17,     10,     10,     10,     10,     
  10,     11,     12,     11,     13,     11,     11,     14,     39,     38,     15,     16,     42,     18,     19,     20,     
  20,     21,     21,     22,     23,     23,     25,     37,     26,     40,     41,     27,     24,     24,     28,     29,     
  30,     28,     28,     28,     31,     4,      31,     31,     31,     33,     32,     43,     35,     35,     35,     35,  
...
char NameStr1[30] = { "" };        // Used in: ListMacro() RemoveMacro() - NameStr1
char NameStr2[30] = { "" };        // Used in: CopyMacro() RenameMacro() - NameStr1 + NameStr2
...
{ uint8_t keycode[6]; 
  ...
  if (Num>26) { if (DoKey16(Num)) return; }; 
  ...
  if (BsDCode1[Num]==0x00) { status("No BSD define"); return; }
...
//                           n=1 Use Flash to load linkfile XnnLink
//                           n=0 Use SDCard to load linkfile XnnLink 
...
{ uint8_t keycode[6] = { 0 };      // simultaneous keys pressed in here
  ...
  while (j<NameStrLen-1) 
        {for (i=0; i<3; i++) { NameStr[i] = inputString[i+j]; } 
         ...
         if (c=='L'||c=='l') { if (a==3) LayerAxD = !LayerAxD; if (a==1||a==2) LayerAxD = a-1;                   // L01Xnn Flash L02Xnn SSDCard
                               for (k=0; k<3; k++) { NameStr1[k] = inputString[i+j+k]; } return true; }          // L00Xnn XnnLink next Link File                                
...
////////////////////////////////////////////////////////////////////////////////////////////
bool DoKey16(byte Num)        // Currently the same as DoKeyMST() but will change in future
////////////////////////////////////////////////////////////////////////////////////////////
...
{ int NameStrLen = 0;
  ...
  for (n=0; n<8; n++) NameStr1[n] = MSTLinkName[n];  // KxxLink + 0x00 length = 8                        
  ...
  do { if (LayerAxD) f = SD.open(NameStr1, "r"); else f = LittleFS.open(NameStr1, "r"); 
       ...
       // Serial.println(NameStrLen); Serial.println(NameStr1); Serial.println(inputString); 
       NameStr1[0] = 0x00; // Only do DoLinkStr() once unless L0nXnn loads new NameStr1      
     } while (NameStrLen>0&&DoLinkStr(NameStrLen)) ;    // Can also stop with 3-char entry such as *** in Link file
  
  return true;
}

////////////////////////////////////////////////////////////////////////////////////////////
bool DoKeyMST(byte Num)        // Currently the same as DoKey16() but will change in future
////////////////////////////////////////////////////////////////////////////////////////////
...
{ int NameStrLen = 0;  
  ...
  for (n=0; n<8; n++) NameStr1[n] = MSTLinkName[n];  // MxxLink + 0x00 length = 8 
     ...
     { PadKeys = true; 
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (OptNum>0) { if (OptNum==1) { MacroUL = !MacroUL; SwitchMacroUL(1); ConfigButtons(1); return; }  
                                         if (OptNum==2) { DoKeyHeldEnable(); ConfigButtons(1); }
...
{ char *ChrPtr;
  ...
  Serial.println("Version: VolumeMacro188 Tobias van Dyk June 2024 License: GPL3");
...
// Use n+1 because filenames are 01-24 
///////////////////////////////////////////////////////////////
{ char macroMST[] = "MSTAx";  // MST1 MST2 0-4
  n++;
  MSTLinkName[1] = 48 + n/10;  MSTLinkName[2] = (n)%10 + 48;   
  MSTLinkName[0] = macroMST[mst] ;
  // Serial.println(MSTLinkName);
...
{ uint8_t keycode[6]; // simultaneous keys pressed in here
  ...
  char status2[]  = {"mstaMSTA"} ;              // MST2 0 1 2 3 -> m s t a
  char NameStr[4] = {"xnn"}  ;  
  File f;
  ...
  status1[16] = status2[MST2+(MacroUL)*4]; 

  if (SrcDst==1||SrcDst==3) sdCard = true;  if (SrcDst==2||SrcDst==0) sdCard = false;  // For Destination 0,2=Flash 1,3=SDCard 
  
  if (MST2==3) { if (!sdCard) f = LittleFS.open(NameStr, "w");      // Only save to file axx if option target is a
                 if (sdCard)  f = SD.open(NameStr, "w");            // Only save to file axx if option target is a
...
{ bool RenOK = false;
  ...
  if (KeyBrdByteNum>0) 
     { while (k!=0x3D){ k = NameStr1[n] = KeyBrdByte[n]; n++; if (n>60) { status("Names too long"); return RenOK; } } NameStr1[n-1] = 0x00; // 0x3D = 
  ...
  if (!LayerAxD) RenOK = LittleFS.rename(NameStr1, NameStr2);    // FlasMem files: Rename 1 to 2 for m s t k files and a00-a99
            else RenOK = SD.rename(NameStr1, NameStr2);          // SDCard files:  Rename 1 to 2 for M S T K files and a00-a99
  
  if (RenOK) status("Macro renamed Src -> Dst"); else status("Use Oldname=Newname or Xnn Znn");
...
{ bool RemOK = false;
  ...
  if (sdCard)  RemOK  = SD.remove(NameStr1); 
...
// Source -> Destination where Source Macro M S T A must already be saved to a file
// Destination Macro can be M S T or A (or K if *cm* used) - use *cm*nnXmm
...
{ char KeyBrd5status[16] =   {"Macro nn -> Xnn"};
  ...
  b = (a==65||a==97)*3 + (a==75||a==107)*4  + (a==77||a==109)*0 + (a==83||a==115)*1 + (a==84||a==116)*2; // kK mM sS tT all ok
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
bool SaveMacro(bool Overwrite)   // Use Only used by *sa,m,s,t then all 24 (U/L case) are saved to flash or SDCard
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{ int m, n;        
  ...
  if (MST1==0) { for (n=0; n<ByteSize; n++) { if (Mtr1to12[Option1][n]==0x00) break;  } MacroM1M12[Option1] = 3; BPtr = Mtr1to12[Option1];  
                      MacroSizeM1M12[Option1] = n; DoFileBytes(1, MSTAName, BPtr, n, sdCard); }  // Save Source File

  if (MST1==1)  { for (n=0; n<ByteSize; n++) { if (Str1to12[Option1][n]==0x00) break; } MacroS1S12[Option1] = 3; BPtr = Str1to12[Option1];  
                      MacroSizeS1S12[Option1] = n; DoFileBytes(1, MSTAName, BPtr, n, sdCard); }  // Save Source File 

  if (MST1==2) { for (n=0; n<ByteSize; n++) { if (Ttr1to12[Option1][n]==0x00) break;  } MacroT1T12[Option1] = 3; BPtr = Ttr1to12[Option1];  
                      MacroSizeT1T12[Option1] = n; DoFileBytes(1, MSTAName, BPtr, n, sdCard); }  // Save Source File 

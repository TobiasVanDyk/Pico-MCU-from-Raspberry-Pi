...
const static int StarCodesMax = 84; // StarCodes Count 16+16+16+16+16+4 StarNum = 0-83
...
{ if (Restore) { status(" ") ;                                                   }                            
  ...
  if (NumKeys) status("Press Red Pads (+) (-) Page Up or Down"); else status(" ");
}
...
{ char *ChrPtr;
  ...
  int c = Button*(Numkeys123+1);                   // nKeys x01-x96,x100-x996 or x001-x996  
  BPtr = MacroBuff; 
  MacroBuffSize = 0;
  MacroBuff[0] = 0x00;
  ...
  // open.
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (!nKeysShow) { a = NKeysX[Button][0];  strcpy(MSTAName, NameStr3);                              // mstak-MSTAK files is macro/string  
                    for (n=0; n<10; n++) if (a==MacroChar[n]) { isMacro = true; Serial.println(MSTAName); if (MacroKeys(c, 3)) return; } }  
  ...
  //  Serial.println(); Serial.print(nFile); Serial.print(" "); Serial.print(NameStr3); Serial.print(" "); Serial.println(nDir);
  
  if (nKeysShow) { status(nFile); return; }  // Only display its nKeys content
...
/////////////////////////////
{
  ...
  if ((Button>11)&&(Layout==2)) 
     { if (OptNum==1 && Button!=16) OptNum=0; // Switch off Pad[o] nKeys char select if Pad[n] nKeys is pressed
       PadKeys = true;        
       if (Button==12) { if (NumKeys) { if (nKeys)  if (Numkeys123<nKeysPage-1)      Numkeys123++; else Numkeys123 = 0; // NumKeys max 8 nKeys max 83 pages
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (NumKeys) { if (Numkeys123>0)  Numkeys123--;
                                        else { if (nKeys)  Numkeys123 = nKeysPage-1;                  // NumKeys max 8 pages nKeys max 83 pages
                                               if (!nKeys) Numkeys123 = NumKeysPageMax-1; }           
                                        NumKeysChange(); ConfigButtons(1); return;}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (row = 0 + rowcount; row < rows; row++) {   // 3 rows 0,1,2 => b = 0+(0*4)=0 1+(0*4)=1 ... 0+(1*4)=4 ....
   ...
   if (NumKeys) { padLabel[5][0] = '+'; padLabel[9][0] = '-'; padColor[5] = Red; 
                  if (nKeysShow) { padLabel[7][0] = 's'; padColor[7] = Red; } else { padLabel[7][0] = 'e'; padColor[7] = Blue; } }
           else { padLabel[5][0] = 's'; padLabel[7][0] = 'm'; padLabel[9][0] = 'o'; padColor[5] = padColor[7] = Blue;  }
...
{ int n;
  ...
  DimVal =        Config1[38];
  nKeys34 =       Config1[39];
  for (n=0; n<20; n++) nDir[n] = Config1[n+40];  // strcpy(nDir, (char *)&Config1[40]);  // Also ok 
  nDirX =         Config1[60];  
}
...
{ int n;
  if (Option==1) { Config1[0] = KeySkip;
                  ...
                  Config1[38] = DimVal; 
                  Config1[39] = nKeys34;                  
                  for (n=0; n<20; n++) Config1[n+40] = nDir[n];  // strcpy((char *)&Config1[40], nDir); // Also ok
                  Config1[60] = nDirX;         
                }
...
{ char *ChrPtr;
  ...
  Serial.print(" nKeys Dir Mode: "); Serial.print(nDirX); Serial.println();
  Serial.print("nKeys Quick-Access:   "); for (int i = 0; i < 10; i++) Serial.print(nKeysCharSet[i]); Serial.println();
      ...
      {  for (n=1;n<12;n++) RecBytes[n] = KeyBrdByte[n+3];
       ...
       { nDirX=0; if (knum>4) { for (n=0; n<knum-4; n++) nDir[n] = KeyBrdByte[4+n]; 
                                     if (k5=='/') { nDir[1]=nChar; nDir[2]='/'; nDir[3]=0x00; nDirX=2; }  // 2: /nChar/
                                             else { nDir[n]='/'; nDir[n+1]=0x00; nDirX=1+(n>1)*2; }  }    // 3: normal /dirname/ 
         if (knum==4||nDirX==1) { nDir[0]='/'; nDir[1]=0x00; nDirX=1; }                                   // 1: default /filename

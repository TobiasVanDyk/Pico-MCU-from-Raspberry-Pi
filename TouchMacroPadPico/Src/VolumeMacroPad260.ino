  ...
  if ((Button>11)&&(Layout==2)) 
     { PadKeys = true; 
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (OptNum>0) { if (OptNum==1) { nKeysCurr[0] = nChar = nKeysCharSet[nCharNum]; nCharNum++; if (nCharNum>9) nCharNum=0;  
                         ...
                         if (NumKeys) { if (Numkeys123<nKeysPage-1) Numkeys123++ ; else Numkeys123 = 0; NumKeysChange(); ConfigButtons(1); return;}
...
{ uint16_t ByteLen = 0;
  ...
  if (!DoWrite)                                 // read data     
     {if (SDCard)  { if (SDFS.exists(STRf))     f = SDFS.open(STRf, "r");      else return 0; }
      ...
      if (f.size() > ByteSize) { f.close(); LargeFile = true; return 0; }
      LargeFile = false;
...
// Works on macros loaded using serial port - they are saved to a file such as <3This is Key S3>
// Only for files/macros < 200 bytes size
/////////////////////////////////////////////////////////////////////////////////////////////////////
{ int i, value;
  byte *BPtr;
  byte a, b;
  ...
  if (MacroBuffSize==0) { status("Macro's not found or too large"); return; }         // Must be <200 bytes and existing file

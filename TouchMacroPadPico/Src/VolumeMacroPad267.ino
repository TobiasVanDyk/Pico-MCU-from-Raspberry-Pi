...
//      37  NormVal 0       DimVal 3         nKeys34 1
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
byte Config1[Config1Size]          = {1,0,1,1,0,0,0,1,'n',8,'n','o','p','q','r','s','t','m','a','k',0,0x0D,0x0A,0, 1, 0, 0, 0, 0, 0, 0, 8, 
                                      0, 0, 2, 0, 6, 0, 3, 1, 0, 0, 0, 0, 0, 0, 0, 0 };
...
const static int StarCodesMax = 83; // StarCodes Count 16+16+16+16+16+3 StarNum = 0-82
...
char MSTAName[6]    = "xnn";     // Filenames a01-a99 m01-m24 s01-s24 t01-t24 from Option1 = 0-23 Option2 = 0-23                             
char MSTName[6]     = "xnn";     // Filenames m01-m24 s01-s24 t01-t24 from Layout = 1-4 
...
{ char *ChrPtr;
  ...
  int c = Button*(Numkeys123+1);                   // nKeys x01-x96 or if nKeysXL more.. 
  BPtr = MacroBuff; 
  MacroBuffSize = 0;
  MacroBuff[0] = 0x00;
     ...
     { if (OptNum==1 && Button!=16) OptNum=0; // Switch off Pad[o] nKeys char select if Pad[n] nKeys is pressed
       ...
       if (Button==16) { if (VarNum>0) { if (VarNum<9) SaveVar = true; else return;  // press [Sav] to save
                         ...
                         if (NumKeys) { if (Numkeys123<nKeysPage-1) Numkeys123++; else Numkeys123 = 0; 
                                        if (!nKeys&&Numkeys123>NumKeysPageMax) Numkeys123 = 0;        // NumKeys max 8 pages nKeys max 83 pages
...
////////////////////////////////////////////////
// Number Pad Page or Macrokeys n01-n144 Select
////////////////////////////////////////////////
void NumKeysChange()
{ int a, b, c, d, n, m, cm1; // Can use itoa() or Timer2Arr() but less flexible than used here with nKeys34 option
  ...
  // for (m = 0; m<12; m++) for (n = 0; n < 6; n++) {Serial.print(NKeysX[m][n], HEX); Serial.print(' '); } Serial.println();                                      
}
...
{ unsigned long T, z;  // t = timeclock struct
  ...
  c99 = b*10 + KeyBrdByte[5]-48; //  00-99
  ...
  switch(c)
      ...
      { if (knum==4) { Config1[37] = NormVal = 0; NormArr[16] = ' '; NormArr[17] = 'F'; }                       // no number 100% PWM
        ...
        if (knum==6) { Config1[37] = NormVal = (c99)*255/100; NormArr[16] = KeyBrdByte[4]; NormArr[17] = KeyBrdByte[5]; }
      ...
      { if (knum==4) { Config1[38] = DimVal = 0; DimArr[16] = '0'; DimArr[17] = '0'; }                       // no number OFF
        ...
        if (knum==6) { Config1[38] = DimVal = (c99)*255/100; DimArr[16] = KeyBrdByte[4]; DimArr[17] = KeyBrdByte[5]; }

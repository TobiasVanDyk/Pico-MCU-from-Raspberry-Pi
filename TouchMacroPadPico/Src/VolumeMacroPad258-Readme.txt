...
Sketch uses 239732 bytes (22%) of program storage space. Maximum is 1044480 bytes.
...
New changes:
1. Added handling of very large strings stored on SDCard through nKeys n,o,p,q,r. If nKeys = m,s,t then large strings also
enabled for M S T keys 01 - 96. Large string tested up to 64kB. Keys MST 01-24 best used for text<200 char and MST 25-96 for large text
or use all other NKeys NOPQR etc 01-96 for large text.
2. Corrected position of check for m s t a k in DoNKeys()
3. Added *Code processing for nKeys: For example with Source p01 construct *bb*7 in the KeyBrd Editor and press [Sav] (not [EXE] that 
...
4. Changed KeyBrd [EXE] and [Sav] to cater for nKeys other than char 'n' - other macro-keyboard-editor functions still need to be tested.
5. Added Undo Redo Ctr+Z Ctr+Y to Macrobank choices
6. Rename *rn*oldname=newname with oldname newname file or /folder names. To test using the keybrd editor choose *rn* with the [*Cm] key, then

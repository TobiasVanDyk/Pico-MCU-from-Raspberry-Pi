...
const static int StarCodesMax = 69; // StarCodes Count 16+16+16+16+5 StarNum = 0-68
...
{ "*am*", "*as*", "*at*", "*bb*", "*bl*", "*br*", "*ca*", "*cm*", "*ct*", "*db*", "*de*", "*e0*", "*e1*", "*e2*", "*e3*", "*e4*", 
  ...
  "*se*", "*sm*", "*ss*", "*st*", "*ta*", "*tb*", "*tp*", "*tt*", "*tw*", "*ua*", "*ul*", "*up*", "*x1*", "*x2*", "*x3*", "*x4*", 
  "*x5*", "*x6*", "*x7*", "*x8*", "*0R*"  };
...
void optionsindicators(int Option) {
  ...
  if (SDNum<22) { if (SDCardArr[2]) { SDNumChar[0] = SD2[SDNum]; tft.setTextColor(SkyBlue1, Black); }    // 0,1 Upper/Lowerscase filename 

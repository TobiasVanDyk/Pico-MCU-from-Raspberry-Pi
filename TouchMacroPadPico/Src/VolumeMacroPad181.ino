...
{ SPI1.setSCK(pinSdClk);
  ...
  if (SDNum>0) SDCardSelectFiles(1);    // SDCard File use is enabled
...
///////////////////////////////////////////
byte FindSDNum(char C)
///////////////////////////////////////////
{char SD1[23]   = { "0123UVWXYZMSTABCDEFGKk" }; 
 char SD2[23]   = { "0123uvwxyzmstabcdefgKk" };
 byte n = 100;
 
 for (byte n=0; n<23; n++) if (C==SD1[n]) return n;
 for (byte n=0; n<23; n++) if (C==SD2[n]) return n; 
...
////////////////////////////////////////////////////////////////////////////////
void SDCardSelectFiles(byte Option)
{ int n, m;
  char SDCardSetX[19] =   {"SDCard Files Set X"};

  if (SDNum==0) { SDCardArr[0] = SDCardArr[1] = 0x00; status("SDCard Files Disabled"); return; }  // SDCard File use is disabled = 0x00 
  
  if (Option) {
  SDCardSetX[17] = SDNum + 48;
  if (SDNum>16&&SDNum<24) SDNum = SDNum - 4;                                                                        // A-G = 17-23 -> 13-19
  if (SDNum>48&&SDNum<56) SDNum = SDNum - 36;                                                                       // a-g = 49-55 -> 13-19
  if (SDNum==29||SDNum==61) SDNum = 10; if (SDNum==35||SDNum==67) SDNum = 11; if (SDNum==36||SDNum==68) SDNum = 12; // m s t M S T
  if (SDNum==27) SDNum = 20; if (SDNum==59) SDNum = 21;                                                             // K k    
  if (SDNum<=21){ status((char *)SDCardSetX); SDCardArr[0] = SDNum; }
           else { SDNum = 0; SDCardArr[1] = 0x00; status("SDCard Incorrect Name"); return; }; }   
  ...
  for (n=0; n<24; n++)   
      {if (SDNum==1) strcpy(SDName[n], SDName1[n]);     // *sd*1
       ...
       /////////////////////////////////////////////////// *sd*4-9 = U-Z+01-24 = U01-U24 to Z01-Z24 
       if (SDNum>3&&SDNum<11) 
          {for (m=0; m<3;  m++) SDName[n][m+1] = SDName3[n][m]; SDName[n][0] = 'U' + SDNum-4; SDCardSetX[17] = 'U' + SDNum-4; } 
       /////////////////////////////////////////////////// *sd*a-g *sd*A-G = A-G+01-24 = A01-A24 to G01-G24   
       if (SDNum>12&&SDNum<20) 
          {for (m=0; m<3;  m++) SDName[n][m+1] = SDName3[n][m]; SDName[n][0] = 'A' + SDNum-13; SDCardSetX[17] = 'A' + SDNum-13; }         
          
       if (SDNum==21) {for (m=0; m<3;  m++) SDName[n][m+1] = SDName3[n][m]; SDName[n][0] = 'K';  }     // *sd*k K01-K24

       if (SDNum==10||SDNum==11||SDNum==12) { DoMSTAName(n, SDNum-10); strcpy(SDName[n], MSTAName); }  // *sd*M,S,T m01-m24 s01-s24 t01-t24
       
       if (SDNum==20) strcpy(SDName[n], K1K6File[n]); }  // *sd*K K01Link - K24Link      
              
    if (Option) SDCardArr[1] = SDCardSetX[17];    // SDCardArr[1] = char " ", 1-3, SDNum 4-9 = U,V,W,X,Y,Z   M,S,T  A,B,C,D,E,F,G   k K        
    Serial.println(SDNum); Serial.println(SDName[0]); Serial.println(SDName[1]); Serial.println(SDCardArr[0]); Serial.println(SDCardArr[1]);
}
//////////////////////
  ...
  switch(c)
      ...
      { if (k2==0x74)                                                // *nt*num 
       ...
       { if (KeyBrdByteNum>5) break;                  // SDNum = single digits or char only after *sd*         
         SDNum = b;                                   // Use the [ADD]ed number to assign 1 - 9 or calc 10 - 21 next       
...
void optionsindicators(int Option) {
  ...
  if (SDNum<22) { SDNumChar[0] = SD1[SDNum];       // SDNumArr[1] = 0x00

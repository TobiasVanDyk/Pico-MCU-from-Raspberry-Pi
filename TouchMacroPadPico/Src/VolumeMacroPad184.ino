...
{ unsigned long T, z;  // t = timeclock struct
  ...
  switch(c)
      ...
      {  for (n=1;n<12;n++) RecBytes[n] = KeyBrdByte[n+3];
       ...
       { if (KeyBrdByteNum>5) b = b*10 + KeyBrdByte[5]-48;      // b = 0, 1-52
         n = k2-48;                                             // n = 0, 1 - 6 or 7,8 for [Del][Ret] key
         if (KeyBrdByteNum<4&&n>0) { status("Add 1-52 0 clear"); delay(1500); break; }   // *x0*0 = clear all but *xn*m n>0 requires m
         if (b>BSDMax)             { status("Invalid Number");   delay(1500); break; }   // Out of range         
         if (n==7) { if (b>0) BsDNum = b-1; else BsDNum = 0; StarOk = true; break; }     // [Del] key options 1-52
         if (n==8) { if (b>0) RetNum = b-1; else RetNum = 8; StarOk = true; break; }     // [Ret] key options 1-52
         if (n==9) { h = 0; for (m=0; m<6; m++) { XNum[m] = ((KeyBrdByte[4+h]-48)*10 + KeyBrdByte[5+h]-48)-1; h = h+2; } 
                     BsDNum = (KeyBrdByte[16]-48)*10 + KeyBrdByte[17]-48; RetNum = (KeyBrdByte[18]-48)*10 + KeyBrdByte[19]-48;  
                     XFiles = true; StarOk = true; break;}      // *x9*m1m2m3s1s2s3b1r1 = all 8 values 1-6 7,8 at once
         if (b>0) { XNum[n-1] = b-1; XFiles = true; } 
         if (b==0||n==0) { for (m=14; m<17; m++) { XNum[m-14]=m; XNum[m-11]=m; }  // *xn*0 Clear 1-6 2 x (Cut Copy Paste)
                           if (n==0) {BsDNum = 0; RetNum = 8;} XFiles = false; }  // *x0*0 Clear 1-8 Bsd + Ret + 2 x (Cut Copy Paste)                 
...
{ 
  ...
  Serial.print("\n"); 
  Serial.println("SDCard Files (Number Name Size):"); ListSDFiles();
}

///////////////////////////////////////////////////
void DoMSTAName(int n, int msta)
///////////////////////////////////////////////////
{ char macroMSTA[] = "mstaxMSTAX";  // MST1 MST2 0-4
  n++;  

...
it will then ignore any assigned strings and use the coded programmed action. Toggle it on/off by pressing the Pad 
once or twice. This functions for all three keysets M, S and T. The A,B,C,D indicator will change to "X" when this 
option is On, and back to A - D when it is Off. For the set of S keys only: If the SDCard option is enabled, the 'X" 
indicator will result in files stored on the SDCard being used when pressing [S1] to [S24].
-----------------------------------------------------------------------------------------------------------------------
...
in any of the Layouts 1-4. Note that when in Layout 2 Cfg it will still be easier to switch to the [Vo] key using the
[Cfg][1L4] key option as there will be at least one Layout change when the the [Vo] or [L1-L34] key is held in.
...
(m) *xn*number n=1-6, number = 1-24. Top row 3 keys cX-Cut, cC-Copy, cV-Paste: Programmable as x1, x2, x3 Layout 1, 
...
To store the string sent on SDCard rather than Flash make sure that "X" instead of "A" to "D" is showing and
Layer 1, 3, or 4 is showing. The sent string will then be stored to the current SDCard file selected set 1,2, or 3
and to the filename selected by the first number after the start <.

Example 1.1: "X" is showing and Layer 3 [S]keys [S7] to [S12] are visible. SDCard set 2 is active (set with 
*sd*2[ADD][EXE]). Then use a serial terminal to send <1This is a string for key [S1].>. This string will then 
be stored in the SDCard file S0007.txt. When key [S7] is pressed with "X" visible then this text will be printed. 
Example 1.2: "C" is showing Layer 3 [S]keys [S13] to [S18] are visible. Then use a serial terminal to send 
...
(3) Note that recent firmware versions are compiled with the early backlight-on commented out in setup(). This means that
    if flash_nuke.uf2 is used to wipe the flash memory the calibration arrows will not be visible. You can either press
    in the 4 corners (upper left, lower left, upper right, lower right) on the blank screen, or you can remove the
    comment for the BL switch-on as shown below, or use an older version of the firmware to calibrate, then load the new
    firmware over it. (The reason for this is because some PC USB ports supply power that keeps the backlight on even if
    the PC is switched off, when the commented line below is active.)
-----------------------------------------------------------------------------------------------------------------------
On First Start: 

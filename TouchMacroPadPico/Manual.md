...
Note: Press [Cfg] once, and then press [Var] repeatedly, to cycle through 24 options for both the default [Del]ete
key top right and the default [Ret]urn key bottom right. If the A-D indicator shows a white A, B. C, or D then [Var]
changes the [Del] key function. If the indicator is orange then [Var] changes the [Ret] key function. Change A-D from
white to orange by pressing the 8-cycle [A-D] key in Layout 2, or for Layouts 1, 3, and 4 by pressing for example,
the [s] Pad in the [S1]-[S24] Layout. Then return to Layout 2 and press [Cfg][Var]. To save the [Var] choice press
the [Sav] key just below it.
...
once or twice. This applies for all three keysets M, S and T. The A,B,C,D indicator will change from white to orange
when this option is On, and back to white when Off. For the set of S keys only: If the SDCard option is enabled, the 
orange A-D indicator will result in files stored on the SDCard being used when pressing [S1] to [S24].
-----------------------------------------------------------------------------------------------------------------------

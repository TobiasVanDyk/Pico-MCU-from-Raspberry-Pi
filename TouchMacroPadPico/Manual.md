...
Note: Press [Cfg] then [Var] repeatedly, to cycle through 24 options for both the default [Del]ete key top right 
and the default [Ret]urn key bottom left. If the A-D indicator shows A,B.C,or D then [Var] changes the [Del] key 
function. If the indicator shows X then [Var] changes the [Ret] key function. Change A-D to X by pressing for 
example the [s] Pad in the [S1]-[S24] Layout - then return to Layout 2 and press [Cfg][Var]. To save the [Var]
choice press the [Sav] key just below it.
...
(m) *xn*number n=1-8, number = 1-24. Top row 3 keys cX-Cut, cC-Copy, cV-Paste: Programmable as x1, x2, x3 Layout 1, 
    ...
    default Cut, Copy,and Paste. Use *x7*n=1-24 for [Del]etekey changes and *x8*n=1-24 for [Ret]urn key changes.

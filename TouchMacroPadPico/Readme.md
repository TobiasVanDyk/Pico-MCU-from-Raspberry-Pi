...
The [**RPi Pico RP2040**](https://www.raspberrypi.org/products/raspberry-pi-pico/) is used as a **Touch LCD Macropad** with direct access to five file-types: (1) file-path-pointers, (2) macros, (3) textstrings, (4) linkfiles, and (5) \*codes. From Layout 2 (Configuration), 8,928 files or "nKeys", grouped as 36 sets of type (1) (aA01-xX96 and 001-996), and 1,440 files of types (2,3,4,5), are directly available (note 3). From Layouts 1, 3, and 4 (Macros M, S and T), nine sets of 24 keys of types 2, 3, 4, 5 (mM01-mM24, sS01-sS24, tT02-T24), as well as another six sets of 99 files (aA01-aA99 and kK01-kK99), are available (note 1). **"Macros" are considered here to be that sub-group of HID touch-keys, touch-buttons, and touch-scrollers, that when touch-pressed sends simultaneous and/or sequential, in (almost) any combination, and length, and with variable speed, HID data and control characters via a USB interface to a PC.** Each of the Macro Layout keys and files, can be further linked through a plaintext list of 3-letter filenames with additional processing such as delay and repeat, storage switching, and chaining  or further linking (note 2). There are also another 456 (19 sets of 24 files), text files which can be accessed from the SDCard using the same macro-keys. Processing of very large text files, tested up to 64kB, but probably only limited by SDFS or LittleFS or FAT32 filesystem size constraints, are available through the nKeys<br> 

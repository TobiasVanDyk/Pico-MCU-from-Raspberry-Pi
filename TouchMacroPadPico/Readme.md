...
The power of these indirected set of files are in the ease in which a large number of actions can be changed with a single folder rename. For example: If there are three folders named AppA, AppB, and AppC, and each folder has 96 specialized macros or strings for the three different applications A, B, and C, then changing the folder name AppA, B, C to that contained in a nKey set such as CurrentApp, will redirect the nKey set of 96 keys to execute the specialized actions for each of the selected Apps A, B, or C.<br>

**Large text file processing:** The nKeys set can handle very large text strings - preferably stored on the SDCard - using nKeys such as those starting with the letters n,o,p,q,r - each with 96 keys on 8 pages. If the nKeys set is m,s, or t then large strings are also enabled for the M S T keys 01 - 96 in Layout 2. Keys MST 01-24 are best used for text<200 char and MST 25-96 for large text  files, or use all NKeys NOPQR etc 01-96 for large text. For example: Copy two large (>20kB) files named L1 and S12 on SDCard. Then program nKey n01 with the content L1. Use the KeyBrd editor with n01 set as the Source - in brown not white - and then do L[Add]1[Add][Sav]. Test by pressing the nKey [n01]. Change the nKey letter-set to S with [Cfg][Opt]PadKey[o] and then press the nKey [S12] - this was tested for both keys both with notepad.<br>
...
This means it is possible with a single macropad keypress to open for example photoshop on a PC, and at the same time re-configure the macropad keys for photoshop-specific functions. See Example 8 in the manual for more detail.<br>

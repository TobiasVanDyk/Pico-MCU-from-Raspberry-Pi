...
/////////////////////////////
{
  ...
  switch(button){
    ...
    case 4: // M1 S1 T1 ///////////////////////////////////////////////// Alt + PrintScreen - some mini keyboards cannot do this
      ...
      i = LayerAB*6;                     // Offset increased by six keys M1-M6 -> M7-M12 
      if (Layout==1) 
         {Serial.print(" M1M7 "); Serial.print(MacroM1M12[0+i]);Serial.print(" ");Serial.print(Mtr1to12[0+i][0]);Serial.print(" ");Serial.println(Mtr1to12[0+i][1]);                    
          if (MacroM1M12[0+i]==2) { 
               if (Mtr1to12[0+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[0+i]; n++) 
                                          { usb_hid.keyboardPress(HIDKbrd, Mtr1to12[0+i][n]); delay(keydelay2);
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[0+i][0]>0x7F) { for (n=0;  n<MacroSizeM1M12[0+i]; n++) { keycode[n] = Mtr1to12[0+i][n]; }
                                            usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } } // Macro1,7==2                   
         keycode[0] = AltL; 
         keycode[1] = PrtScr;
    ...
    case 5: // M2 S2 T2 ///////////////////////////////////////////////// Open admin powershell including UAC completion
      ...
      i = LayerAB*6;                 // Offset increased by six keys M1-M6 -> M7-M12                                                                           
      if (Layout==1) 
         {Serial.print(" M2M8 "); Serial.print(MacroM1M12[1+i]);Serial.print(" ");Serial.print(Mtr1to12[1+i][0]);Serial.print(" ");Serial.println(Mtr1to12[1+i][1]);
          if (MacroM1M12[1+i]==2) 
             { if (Mtr1to12[1+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[1+i]; n++) 
                                          { usb_hid.keyboardPress(HIDKbrd, Mtr1to12[1+i][n]); delay(keydelay2);
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[1+i][0]>0x7F) { for (n=0;  n<MacroSizeM1M12[1+i]; n++) { keycode[n] = Mtr1to12[1+i][n]; }
                                            usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } } // Macro2,8==2   
    ...
    case 6: // M3 S3 T3 /////////////////////////////////////////////// Open Run window with last coommand visible and selected
      ...
      if (Layout==1) 
         {if (MacroM1M12[2+i]==2) 
             { if (Mtr1to12[2+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[2+i]; n++) 
                                          { usb_hid.keyboardPress(HIDKbrd, Mtr1to12[2+i][n]); delay(keydelay2);
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[2+i][0]>0x7F) { for (n=0; n<MacroSizeM1M12[2+i]; n++) { keycode[n] = Mtr1to12[2+i][n]; }
                                            usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } } // Macro3,9==2               
    ...
    case 8: // M4 S4 T4 ///////////////////////////////////////////// Send a text or macro - could overide by sending serial 0text or macro
      ...
      if (Layout==1) 
         {if (MacroM1M12[3+i]==2) 
             { if (Mtr1to12[3+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[3+i]; n++) 
                                       {usb_hid.keyboardPress(HIDKbrd, Mtr1to12[3+i][n]); delay(keydelay2);
                                        usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[3+i][0]>0x7F) { for (n=0;  n<MacroSizeM1M12[3+i]; n++) { keycode[n] = Mtr1to12[3+i][n]; }
                                        usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                        usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } } // Macro4,10==2      
    ...
    case 9: // M5 S5 T5 //////////////////////////////////////////// Open normal command prompt via Run window
      ...
      if (Layout==1) 
         {if (MacroM1M12[4+i]==2) 
             { if (Mtr1to12[4+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[4+i]; n++) 
                                          { usb_hid.keyboardPress(HIDKbrd, Mtr1to12[4+i][n]); delay(keydelay2);
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[4+i][0]>0x7F) { for (n=0;  n<MacroSizeM1M12[4+i]; n++) { keycode[n] = Mtr1to12[4+i][n]; }
                                            usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } } // Macro5,11==2    
    ...
    case 10: // M6 S6 T6 //////////////////////////////////////// M6 text or Enter key LayerAB==1 b = 5; else b =0; 
      ...
      if (Layout==1) 
         {if (MacroM1M12[5+i]==2)
             { if (Mtr1to12[5+i][0]<0x80) { for (n=0;  n<MacroSizeM1M12[5+i]; n++) 
                                          { usb_hid.keyboardPress(HIDKbrd, Mtr1to12[5+i][n]); delay(keydelay2);
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2); } break; }                 
               if (Mtr1to12[5+i][0]>0x7F) { for (n=0;  n<MacroSizeM1M12[5+i]; n++) { keycode[n] = Mtr1to12[5+i][n]; }
                                            usb_hid.keyboardReport(HIDKbrd, 0, keycode);      delay(keydelay2); 
                                            usb_hid.keyboardRelease(HIDKbrd);                 delay(keydelay2);   break; } }  // Macro6,12==2 
...
void optionsindicators() {
  ...
  if (Option1<12)             tft.drawString(num16[Option1], OPT_X+110, OPT_Y); else { tft.drawString("  ", OPT_X+110, OPT_Y); }

...
uint8_t dbArr[10] = {0, 4, 8,  16, 24, 32, 48, 64, 96, 128};
uint8_t bbArr[10] = {0, 8, 16, 24, 32, 40, 48, 64, 96, 128};
long unsigned int tbArray[10] = {600000000, 30000, 60000, 90000, 120000, 180000, 300000, 600000, 1800000, 3600000 };
...
/////////////////////////////////////////////////////////////////////////////////////////////
#define PAD_W 20          // 20          // Key width  PAD_X is middle -> Pad X is from 317 to 303 
#define PAD_H 30          // 24          // Key height
#define PAD_SPACING_Y 48  // 30  // was 74 for 3 pads // Next below 
#define PAD_X 455         // 300         // config buttons right side
#define PAD_Y 44           // config buttons top
//////////////////////////////////////////////////////////////////////////////////
...
//        uncoditionally-false and or xor not proves models  
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
char DimArr[]      = "Dimmer Val          (x/256)";
char NormArr[]     = "LCD Bright          (x/256)";
...
char MathChr[32];                // Current Math description
...
void setup() {
  ...
  BackLightOn = true;      // TFT init have turned it on
...
{ if (Restore) { if (Pbutton==3) { VolDisable = VolDisablePrev; MuteDisable = MuteDisablePrev;} }                              
                 ...
                 if (Pbutton==5) { Math = Media = NumKeys = Kbrd      = false;}  }
  ConfigButtons(1);
...
/////////////////////////////
{
  ...
  if ((button>11)&&(Layout==2)) 
     {PadKeys = true; 
      ...
      if (button==16) { status("Not assigned"); return; }       
      Kbrd = !Kbrd; PadKeysState(button-11, !Kbrd);  return;                                       
     }
  ...
  switch(button){
    case 0: ///////////////////////////////////////////////// Cut = control + x   
      ...
      //keycode[0] = CtrL;      // Also works fine doing it this way
      //keycode[1] = HID_KEY_X;  
    ...
    case 10: // M6 S6 T6 //////////////////////////////////////// M6 text or Enter key LayerAB==1 b = 5; else b =0; 
      ...
      */
      ...
      tud_task();        // call before releasing the keys
      keycode[0] = 0x00;
      tud_hid_keyboard_report(HIDKbrd, 0x00, keycode); break;
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  if (Math) { for (m=0; m<12; m++) {strcpy(keyLabel[m], MathLabel[MathX][m]);}
              for (m=0; m<11; m++) {keyColor[m] = MathColour1;} keyColor[3]=keyColor[7]=keyColor[11]=MathColour2; }

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (Kbrd) {for (m=0; m<12; m++) {strcpy(keyLabel[m], KbrdLabel[KeyBrdX][m]);}
             for (m=0; m<12; m++) {keyColor[m] = CfgColour;}; goto KeyBrdsDone; }

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (MouseKeys) { for (m=0; m<12; m++) {strcpy(keyLabel[m], Mousekeys1[m]);}
                   for (m=0; m<12; m++) {keyColor[m] = Pink;}; goto KeyBrdsDone; }
                  
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (NumKeys)   { for (m=0; m<12; m++) {strcpy(keyLabel[m], Numkeys1[m]);}
...
{ char *ChrPtr;
  ...
  Serial.println("VolumeMacroPadILI9486-3");
  ...
  ReadLCDDim(); b = (DimVal*100)/256;
  Serial.printf("LCD Dimmed: %d percent (%d/256)\n", b, DimVal);  

  ReadLCDBright(); b = (NormVal*100)/256;
  Serial.printf("LCD Brightness: %d percent (%d/256)\n", b, NormVal);
...
{ char PArray[4][10] = {"tRestart", "TRestart", "tPowerOff", "TPowerOff"};
  ...
  status(RPArr[Option]); SendBytesEnd(); 
  ...
  // if (Macro1==1)         status("[M1] = Macro1"); 
  //                        status("Macro1 Press [Up] save [M1]");
  // if (Macro2==1)         status("[M2] = Macro2"); 
  //                        status("Macro2 Press [Up] save [M2]");
  // if (Math)              status("Math exit");  
  // if (Math)              status("Math symbol sent");
  // if (button==16)        status("Not assigned");
  // if (Rotate180)         status("Restart to rotate 180 degrees");
  // 
...
{ unsigned long t, z;
  ...
  if ((KeyBrdByte[1]==0x72)&&(KeyBrdByte[2]==0x6F))     // *ro* = rotate 180 degrees turn on
     {Rotate180 = true; 
      ...
      status("Restart to rotate 180 degrees"); return; }      
  ...
  if ((KeyBrdByte[1]==0x70)&&(KeyBrdByte[2]==0x6F))     // *po* = Power Keys Menu Option 
     {PowerKeysMenu = !PowerKeysMenu; if (PowerKeysMenu) status("PowerKeys Menu ON"); else status("PowerKeys Menu OFF"); return; }

  if ((KeyBrdByte[1]==0x63)&&(KeyBrdByte[2]==0x61))     // *ca* = Calibration On/Off 
     {DoCal = !DoCal; if (DoCal) { status("Calibrate ON - send *ca* OFF"); 
                                   File f = LittleFS.open("DoCal", "w"); if (f) {f.print(DoCal); f.close(); } return; } 
                            else { status("Calibrate OFF - send *ca* ON"); LittleFS.remove("DoCal"); return; }  }

  b = (KeyBrdByte[4]-48);    // b = 0 - 9
  if (b >9) return;          // do nothing if not
                      
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // (1) *tb*num = blank lcd timemout          *tb*0,1-9 = infinite,30,60,90,120,240 sec  100 min max  0 = 7 days
  //                                       0=7 days   1=30s  2=60s  3=90s  4=2min  5=3min  6=5min  6=10min 7=15min  8=30min  8=60min
  //      long unsigned int tbArray[10] = {600000000, 30000, 60000, 90000, 120000, 180000, 300000, 600000, 900000,  1800000, 3600000 };
  // (2) *db*num = dim backlight pwm           *db*0,1-9 = on/off,8,16,24,32,40,48,56,64,72
  //     uint8_t dbArr[10]  = {0, 4, 8,  16, 24, 32, 48, 56, 64, 96, 128};
  // (3) *bb*num = normal level backlight pwm  *bb*0,1-9 = on/off,8,16,24,32,40,48,56,64,72
  //      uint8_t bbArr[10] = {0, 8, 16, 24, 32, 40, 48, 56, 64, 96, 128};
  ...
  if ((KeyBrdByte[1]==0x64)&&(KeyBrdByte[2]==0x62))                    // *db*num = dim backlight pwm  *db*0,1-9
      {DimVal=dbArr[b];                                                // 
       z = DimVal;  i = 3;                                             // 3 digits
       while (i!=0) { DimArr[i+12] = (z%10) + 48; z = z/10; i--; }     // modulus (%) and division (/) operations 
       File f = LittleFS.open("LCDDimmer", "w");
       if (f) {f.print(DimVal); f.close(); }                           // Stored as 1-3 ASCII number bytes 48 48 56 = 008 = 8/256 %
       Serial.println(DimVal);
       SendBytesEnd(); status(DimArr); return; }      

  if ((KeyBrdByte[1]==0x62)&&(KeyBrdByte[2]==0x62))                    // *bb*num = normal backlight pwm  *bb*0,1-9 0 8/256 - 128/256 % PWM
      {NormVal=bbArr[b];                                               // 0, 8/256 - 128/256 % PWM
      if (NormVal==0) digitalWrite(LCDBackLight, HIGH);                // Backlight Full ON
                else  analogWrite(LCDBackLight, NormVal);              // Backlight Brightness ON
       z = NormVal;  i = 3;                                            // 3 digits
       while (i!=0) { NormArr[i+12] = (z%10) + 48; z = z/10; i--; }    // modulus (%) and division (/) operations 
       File f = LittleFS.open("LCDBrightness", "w");
       if (f) {f.print(NormVal); f.close(); }                          // Stored as 1-3 ASCII number bytes 48 48 56 = 008 = 8/256 %
       Serial.println(NormVal);
       SendBytesEnd(); status(NormArr); return; }      
       
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // (1) Default time values: Short Time = 30 seconds Long Time 600 seconds (10 minutes). Change Time values send the macro with
  // the built-in keyboard for PowerOff *ot*num or *oT*num or Restart *rt*num or *rT*num where num 0 = 600 seconds (10 or 1000
  // minutes) 1 = 30 second (1000 sec for T) to 9 = 270 seconds (150 minutes for T).
  // (2) Send the macro *po* with the built-in keyboard to toggle the Power Keys Option to use the Menu (GUI+X + i,u,r or the 
  // GUI+R + shutdown + options command.
  // (3) To cancel a timed shutdown press the [Stop] key. Tooo exit the Power Restart Keypad press any of the [ ] non-functional
  // keys or press [Cfg][Rof] again.
  // (4) Short time (t): (num=1-9)*30   = 30-270    sec ( 0.5 - 4.5 min)           t>600 sec t=current t       num=0 t=600 sec
  //      Long time (T): (num=1-9)*1000 = 1000-9000 sec (16.7 - 150 min (2.5 hrs)) t>60000 seconds t=current t num=0 t=60000 sec (1000 min (16.7 hrs))
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   if (KeyBrdByte[1]==0x6F)
      { if (KeyBrdByte[2]==0x74)                                       // *ot*num 
        ...
        if (KeyBrdByte[2]==0x54)                                       // *oT*num 
           ...
           WritePowerKeys(TimePowerOff, 3) ;} return; }                // "tRestart", "TRestart", "tPowerOff", "TPowerOff"
   ...
   if (KeyBrdByte[1]==0x72)
      { if (KeyBrdByte[2]==0x74)                                       // *rt*num 
           ...
           WritePowerKeys(timeRestart, 0) ;}                           // "tRestart", "TRestart", "tPowerOff", "TPowerOff"

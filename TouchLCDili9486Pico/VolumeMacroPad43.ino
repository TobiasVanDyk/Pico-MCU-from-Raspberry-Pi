...
{ "@", "#", "?", "$", "%", "^", "&", ",", ".", "~", "`", "\\", "\'", "\"", "|", ":", ";" };
const static char StarCode[16][5] =    
{ "*bb*", "*db*", "*tb*", "*ot*", "*oT*", "*rt*", "*rT*", "*po*", "*cm*", "*ca*", "*po*", "*fo*", "*ul*", "*ua*", "*kb*", "*mk*" };
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{   byte p, n, m, b;
   ...
   Serial.println();
   for ( m = 0; m<12; m++ )   // char LinkNameM[12][10][8] 
...
{ char *ChrPtr;
  ...
  /*
  //File f = LittleFS.open("LinkNameM", "w");
  //for (n=0; n<12; n++) if (LinkM[n]>0) 
  //File f = LittleFS.open("LinkNameM", "w"); if (f) {f.write( (const unsigned char *)LinkNameM, 960);} f.close(); 
  File f = LittleFS.open("LinkNameM", "w"); 
       for (n=0; n<12; n++) 
            for (m=0; m<10; m++) { StrLen = strlen(LinkNameM[n][m]); f.print(LinkNameM[n][m]); f.print('\0'); } f.close(); 
  */     
}
/////////////////
void ReadLinks()
...
{ char *ChrPtr;
  ...
  // char LinkNameM[12][10][8] Must do 10 strings (filenames) for each of 12 M keys
  n = 0; m = 0;
  //File f = LittleFS.open("LinkNameM", "r"); while (f.available()) { LinkNameM[n][m][n] = f.read(); n++; } f.close(); 
  //if (LittleFS.exists("LinkNameM")) { ChrPtr =  * LinkNameM[0]; File f = LittleFS.open("LinkNameM", "r"); n = 0;  // f write 2290
  //    while (f.available()) { ChrPtr[n] = f.read(); n++; } f.close(); }
  //File f1 = LittleFS.open("LinkM", "r"); ASize = f1.size(); f1.readBytes((char *)LinkM, ASize);  f1.close();
  ...
  //File f2 = LittleFS.open("LinkNameM", "r"); ASize = f2.size(); f2.readBytes((char *)LinkNameM, ASize);  f2.close();
 
  //LinkM[0] = LinkM[1] = LinkM[2] = 1;   // testing....
}
////////////////////////////////
...
{ unsigned long t, z;
  ...
  if (KeyBrdByte[1]==0x6c&&KeyBrdByte[2]==0x72)         // *lr* = Read Links from Flash
     {ReadLinks(); status("Links Read from Flash"); return true; }  
  if (KeyBrdByte[1]==0x6c&&KeyBrdByte[2]==0x77)         // *lw* = Save Links to Flash
     {SaveLinks(); status("Links Saved to Flash");  return true; }  
  if (KeyBrdByte[1]==0x6c&&KeyBrdByte[2]==0x70)         // *lp* = Print Links
...
{   int i, n;
    ...
    if (KeyBrdX==2) {if (button==10) {a = KeyBrdFx[KeyBrdF1F24]; Fx = true;}             // Xlate F1-F24 - see if (a>0) below where it is used 
                     ...
                     if (button==6)  { for (n=0; n<4; n++) { b = StarCode[StarNum][n]; KeyBrdByte[n] = b; KBDisp[n] = b; } 
                                       ...
                                       StarNum++; if (StarNum==16) StarNum = 0;      return; } }

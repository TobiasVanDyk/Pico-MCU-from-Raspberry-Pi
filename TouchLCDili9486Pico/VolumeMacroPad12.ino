...
int  Macro1 = 0;                 // Ststus of KeyBrd Macros 1 = has been saved, 2 = use for M1 M2
int  Macro2 = 0;                 // Ststus of KeyBrd Macros 1 = has been saved, 2 = use for M1 M2
int  Macro1Size = 0;             // Ststus of KeyBrd Macros 1 = has been saved, 2 = use for M1 M2
int  Macro2Size = 0;             // Ststus of KeyBrd Macros 1 = has been saved, 2 = use for M1 M2
...
{ uint8_t keycode[6] = { 0 }; // simultaneous keys pressed in here
  ...
  if ((!Fx)&&(KeyBrdByte[0]<0x80))                                                  // F1-F12 must be sent with keycode not keyPresssequence via keyPress 
     {for (n = 0; n < KeyBrdByteNum; n++) if (KeyBrdByte[n]<0x80)                   // When 1st char is normal alphanumeric all must be <0x80 
      ...
      status("Macro 1 Press [Up] save [M1]"); Macro1 = 1; Macro1Size = KeyBrdByteNum;}
  ...
  if ((Fx)||(KeyBrdByte[0]>0x7F)&&(KeyBrdByteNum<6))
     {for (n = 0; n < KeyBrdByteNum; n++) {keycode[n] = KeyBrdByte[n]; MacroM2[n] = KeyBrdByte[n]; }  
      ...
      if (f) {f.write(KeyBrdByte, KeyBrdByteNum); f.print('\0'); f.close(); }  
      status("Macro 2 Press [Up] save [M2]"); Macro2 = 1; Macro2Size = KeyBrdByteNum;}
      /*
...
void optionsindicators() {
  ...
  if (Option1)                tft.drawString("1", OPT_X+110, OPT_Y);  else { tft.drawString(" ", OPT_X+110, OPT_Y); }
  if (Option2)                tft.drawString("2", OPT_X+220, OPT_Y); else { tft.drawString(" ", OPT_X+220, OPT_Y); }
  if (KeyFontBold) tft.setFreeFont(&FreeSansBold12pt7b);
              else tft.setFreeFont(&FreeSans12pt7b);   
}
////////////////////////////////////////////////////////////////////////////

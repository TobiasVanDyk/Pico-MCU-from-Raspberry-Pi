...
{"abc",  "def",  "ghi",  "EXE", "jkl", "mno", "pqr", "NXT", "stu", "vwx", "yz ", "ADD",    // ASCII->HID Subtract  dec
 ...
 "012",  "345",  "678",  "EXE", "9+-", "*/=", "@#?", "NXT", "Sym", "Brc", "Fnn", "ADD",    // 1-9 subtract 19 dec 0 subtract 9 dec
 "KpN",  "Ren",  "Rmv",  "EXE", "*Cm", "Cpy", "Lnk", "NXT", "Src", "Dst", "Num", "ADD",    // *Cm=StarCodes Lst=MacroContent Rmv=RemoveMacro
...
{ "[", "]", "(", ")", "{", "}", "<", ">" };
const static char KeyBrdSymbols[14][2] =    
{ "$", "%", "^", "&", ",", ".", "~", "`", "\\", "\'", "\"", "|", ":", ";" };
const static char StarCode[14][5] =    
{ "*cm*", "*bb*", "*db*", "*tb*", "*ot*", "*oT*", "*rt*", "*rT*", "*po*", "*ca*", "*po*", "*fo*", "*kb*", "*mk*" };
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int  MacroM1M12[12] = { 0 };     // Value 0 = values assigned 1 = assigned values 2 = saved
int  MacroS1S12[12] = { 0 };
int  MacroT1T12[12] = { 0 };

int  MacroSizeM1M12[12] = { 0 }; // Byte size of macro of assigned + saved macro
int  MacroSizeS1S12[12] = { 0 }; 
int  MacroSizeT1T12[12] = { 0 }; 

int  LinkM[12] = { 0 };          // 0 - 9 Number of linked macro's for each Key 1-12
int  LinkS[12] = { 0 };
int  LinkT[12] = { 0 };

char LinkNameM[12][10][8] = { 0 };  // File names on Flash of linked macro's -> Macro01, Macro02
char LinkNameS[12][10][8] = { 0 };  // [12] keys for each M S T [10] linked macrofilenames length 8 chars  0x00
char LinkNameT[12][10][8] = { 0 };  // Linked filenames always MacroNN NN = 01 - 99
...
int MST1 = 4;                    // Source Macro MST1 = 0 1 2 3 4  = M S T mAcro not defined keys - macro copy + link
int MST2 = 4;                    // Destination Macro MST2 = 0 1 2 3 4  = M S T mAcro not defined keys - macro copy + link
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
byte Str1[StrSize],  Str2[StrSize],  Str3[StrSize],  Str4[StrSize],   Str5[StrSize],   Str6[StrSize];
byte Str7[StrSize],  Str8[StrSize],  Str9[StrSize],  Str10[StrSize],  Str11[StrSize],  Str12[StrSize];
byte Ttr1[StrSize],  Ttr2[StrSize],  Ttr3[StrSize],  Ttr4[StrSize],   Ttr5[StrSize],   Ttr6[StrSize];
byte Ttr7[StrSize],  Ttr8[StrSize],  Ttr9[StrSize],  Ttr10[StrSize],  Ttr11[StrSize],  Ttr12[StrSize];
...
{uint8_t keycode[6] = { 0 };     // simultaneous keys pressed in here
 ...
 if (Layout==3) { if (Str1to12[c][0]==0x00) return false; // Assume NULL as first byte means no content  
...
if (Layout==4) { if (Ttr1to12[c][0]==0x00) return false; // Assume NULL as first byte means no content  
 ...
 if (Layout==1) { if (Mtr1to12[c][0]==0x00) return false; // Assume NULL as first byte means no content                    
                  if (MacroM1M12[c]==2) { if (Mtr1to12[c][0]<0x80) { for (n=0;  n<MacroSizeM1M12[c]; n++) 
...
/////////////////////////////
{
  ...
  if ((button>11)&&(Layout==2)) 
     {PadKeys = true; 
      ...
      if (button==16) { if (Kbrd) {KeyBrdDirect = !KeyBrdDirect; optionsindicators(); 
  ...
  switch(button){
    ...
    case 4: // M1 S1 T1 ///////////////////////////////////////////////// Alt + PrintScreen - some mini keyboards cannot do this           
         if (Layout>2) if (MacroKeys(c)) break; 
               
         if (Layout==1) 
            { if (LinkM[c]==0) { if (MacroKeys(c)) break; }
              if (LinkM[c]>0)  { if (MacroLink(c)) break; }
  ...
  switch(button){
    ...
    case 1: // ARROW UP ////////////////////////////////////////////////
      ...
      if (MacroM1M12[Option1]==1)       // macro 1-12 (Option1 select) saved to M1-12 M1 to M12 (Option2 select)
         {status("Macro OK"); 
          for (n=0; n<MacroSizeM1M12[Option1]; n++) { Mtr1to12[Option2][n] = MacroM1to12[Option1][n]; } 
          Mtr1to12[Option2][n] = 0x00; MacroM1M12[Option1] = 2; 
          BPtr = Mtr1to12[Option2];     // BPtr = MacroM1to12[Option1];
          DoFileBytes(1, MTRname[Option2], BPtr, MacroSizeM1M12[Option1]); break; }
  ...
  if (LittleFS.exists("KeyFontBold"))      KeyFontBold = true;  else KeyFontBold = false; } // Button Font Bold/Normal labels
   ...
   for (n=0; n<12; n++) 
      {NameStr = STRname[n]; BPtr = Str1to12[n]; MacroS1S12[n] = 0;               // ByteSize is max - ok DoFileBytes read until eof
       if (LittleFS.exists(NameStr)) { MacroSizeS1S12[n] = DoFileBytes(ByteOK, NameStr, BPtr, ByteSize);  MacroS1S12[n] = 2; } }
           
  for (n=0; n<12; n++) 
      {NameStr = TTRname[n]; BPtr = Ttr1to12[n];  MacroT1T12[n] = 0;              // ByteSize is max - ok DoFileBytes read until eof    
       if (LittleFS.exists(NameStr)) { MacroSizeT1T12[n] = DoFileBytes(ByteOK, NameStr, BPtr, ByteSize); MacroT1T12[n] = 2; } }
           
  for (n=0; n<12; n++) 
      {NameStr = MTRname[n]; BPtr = Mtr1to12[n];                                  // ByteSize is max - ok DoFileBytes read until eof         
       if (LittleFS.exists(NameStr)) MacroSizeM1M12[n] = DoFileBytes(ByteOK, NameStr, BPtr, ByteSize); }
   
  for (n=0; n<12; n++) 
      {NameStr = KBMacro[n]; BPtr = MacroM1to12[n]; MacroM1M12[n] = 0;            // ByteSize is max - ok DoFileBytes read until eof 
       if (LittleFS.exists(NameStr)) { MacroSizeM1M12[n] = DoFileBytes(ByteOK, NameStr, BPtr, ByteSize); MacroM1M12[n] = 2; } }  // actual size
...
// Destination is Option 2 i.e. [Dst] = [Mnn] always Key RH number in KeyBrd
//
// Example 1: Set up MM 01 01 as SD SrcNum DestNum - during same session press [CTR][SHF][Esc][EXE][UP] - save to M01
//            Can check in Terminal File list has Macro01 4bytes MtrData1 4bytes (only autosave for 01)
//            Press [M1] and TaskMan opens
// Example 2: Set up MM 01 02 - press [GUI][r][EXE][Up] - save to M02 - filelist only has Macro02 3bytes
//            Must therefore copy it to M02
//            Set up AM 02 02 - then *cm* [EXE] - file list now has MtrData2 4 (added 0x00) press [M2] open run command window
// Example 3: Set up AS 02 01 - then *cm* [EXE] - file list now has StrData1 4 press [S1] open run command window
// Example 4: Set up AS 01 02 - then *cm* [EXE] - file list now has StrData2 5 press [S2] open TaskMan
//
...
{ uint8_t keycode[6] = { 0 }; // simultaneous keys pressed in here
  ...
  if (Option2<10) {status1[22] = num16[Option2][0];} else {status1[21] = (Option2/10)-48; status1[22] = num16[Option2][1];}
  ...
  if ((!Fx)&&(KeyBrdByte[0]<0x80))                                              // F1-F24 must be sent with keycode not keyPress
     { for (n = 0; n < KeyBrdByteNum; n++) if (KeyBrdByte[n]<0x80)              // When 1st char is normal alphanumeric all must be <0x80    
           {usb_hid.keyboardPress(HIDKbrd, KeyBrdByte[n]); delay(keydelay);     // Converted via end of hid.h from ascii to hid
            ...
            MacroM1to12[Option2][n] = KeyBrdByte[n];}
  ...
  if ((Fx)||(KeyBrdByte[0]>0x7F)&&(KeyBrdByteNum<6))                            // Fnn = 3A-45 + 68-73 hex
     { for (n = 0; n < KeyBrdByteNum; n++) {keycode[n] = KeyBrdByte[n]; MacroM1to12[Option2][n] = KeyBrdByte[n]; }  
           usb_hid.keyboardReport(HIDKbrd, 0, keycode); delay(keydelay);
           usb_hid.keyboardRelease(HIDKbrd);            delay(keydelay);  
       File f = LittleFS.open(NameStr, "w");
       if (f) {f.write(KeyBrdByte, KeyBrdByteNum); f.print('\0'); f.close(); }  
       status(status1); MacroM1M12[Option2] = 1; MacroSizeM1M12[Option2] = KeyBrdByteNum;}
  
  // Note also saves: DoFileBytes(1, MTRname[Option2], BPtr, MacroSizeM1M12[Option1]) in buttons() case [Up] 
...
bool LinkMacro()
//////////////////////////////////////////////////////
// Two stage 1. Link two MacroNum1 to MacroNum2
//              Use only Option1 Option2 not SD
//           2. Assoc linked macro to M S T key
//              Use SD Source M S T [Add] 1-12 
//////////////////////////////////////////////////////             
{ byte b;
  bool LinkOK = true;
...
{ char KeyBrd5status[16] =   {"Macro nn -> Xnn"};
  ...
  if (MST1==3) { NameStr = KBMacro[Option1]; }                            
  if (MST1==0) { NameStr = MTRname[Option1]; }
  if (MST1==1) { NameStr = STRname[Option1]; } 
  if (MST1==2) { NameStr = TTRname[Option1]; }

  BPtr = MacroBuff; MacroBuffSize = DoFileBytes(0, NameStr, BPtr, ByteSize);          

  if (MST2==0) { for (n=0; n<MacroBuffSize; n++) { Mtr1to12[Option2][n] = MacroBuff[n]; } 
                 MacroM1M12[Option2] = 2; BPtr = Mtr1to12[Option2];  DoFileBytes(1, MTRname[Option2], BPtr, MacroBuffSize); }  // Save Destination File

  if (MST2==1) { for (n=0; n<MacroBuffSize; n++) { Str1to12[Option2][n] = MacroBuff[n]; } 
                 MacroS1S12[Option2] = 2; BPtr = Str1to12[Option2];  DoFileBytes(1, STRname[Option2], BPtr, MacroBuffSize); }  // Save Destination File  

  if (MST2==2) { for (n=0; n<MacroBuffSize; n++) { Ttr1to12[Option2][n] = MacroBuff[n]; } 
                 MacroT1T12[Option2] = 2; BPtr = Ttr1to12[Option2];  DoFileBytes(1, TTRname[Option2], BPtr, MacroBuffSize); }  // Save Destination File

  if ((MST==0)&&(MacroBuffSize>0)) status("Macro Copied check file list"); else status("Macro's not found");
...
{   int i, n;
    ...
    if (KeyBrdX==2) {if (button==10) {a = KeyBrdFx[KeyBrdF1F24]; Fx = true;}             // Xlate F1-F24 - see if (a>0) below where it is used 
                     ...
                     if (button==8)  {a = b = KeyBrdSymbols[SymbolsNum][0]; SymbolsNum++; if (SymbolsNum==14) SymbolsNum=0; } }
          
    if (KeyBrdX==3) {if (button==1)   { RenameMacro(); SendBytesEnd(0); InitCfg(0);   return; }    // Could remove SendBytesEnd(0); 
                     if (button==2)   { RemoveMacro(); SendBytesEnd(0); InitCfg(0);   return; }    // Could remove SendBytesEnd(0); 
                     if (button==4)   { for (n=0; n<4; n++) {b = StarCode[StarNum][n]; KeyBrdByte[n] = b; KBDisp[n] = b; } 
                                        KeyBrdByteNum = 4; KBDispPos = 4; status((char *)KBDisp); 
                                        StarNum++; if (StarNum==14) StarNum = 0;      return; }
                     if (button==5)   { CopyMacro(); SendBytesEnd(0); InitCfg(0);     return; }    // Could remove SendBytesEnd(0); 
                     if (button==6)   { LinkMacro(); SendBytesEnd(0);                 return; }    
...
void optionsindicators() {
  int ColArr[5] = {Cyan, Yellow, Green, White, Red};
  ...
  if (!Kbrd)                  tft.drawString(" ", OPT_X, OPT_Y);                    // Must come after the previous line

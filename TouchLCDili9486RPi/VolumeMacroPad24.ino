...
{"abc",  "def",  "ghi",  "EXE", "jkl", "mno", "pqr", "NXT", "stu", "vwx", "yz ", "ADD",    // ASCII->HID Subtract  dec
 ...
 "KpN",  "Lst",  "Rmv",  "EXE", "*Cm", "Cpy", "Lnk", "NXT", "Src", "Dst", "Num", "ADD",    // *Cm=StarCodes Lst=MacroContent Rmv=RemoveMacro
...
int  MacroM1M12[12] = { 0 };     // Value 0 no values assigned 1 = assigned values 2 = saved
int  MacroS1S12[12] = { 0 };
int  MacroT1T12[12] = { 0 };

int  MacroSizeM1M12[12] = { 0 }; // Byte size of macro
...
{ char KeyBrd5status[16] =   {"Macro nn -> Xnn"};
  ...
  if (MST1==3) { NameStr = KBMacro[Option1]; BPtr = MacroM1to12[Option1];                                         // Read Source File macro00-99
                 MacroSizeM1M12[Option1] = DoFileBytes(0, NameStr, BPtr, ByteSize); MacroM1M12[Option1] = 2; }    // ByteSize is max - ok DoFileBytes read until eof

  if (MST1==0) { NameStr = MTRname[Option1]; BPtr = Mtr1to12[Option1];                                            // Read Source File M1 - M12
                 MacroSizeM1M12[Option1] = DoFileBytes(0, NameStr, BPtr, ByteSize); MacroM1M12[Option1] = 2; } 

  if (MST1==1) { NameStr = STRname[Option1]; BPtr = Str1to12[Option1];                                            // Read Source File S1 - S12
                 MacroSizeS1S12[Option1] = DoFileBytes(0, NameStr, BPtr, ByteSize); MacroS1S12[Option1] = 2; } 

  if (MST1==2) { NameStr = TTRname[Option1]; BPtr = Ttr1to12[Option1];                                            // Read Source File T1 - T12
                 MacroSizeT1T12[Option1] = DoFileBytes(0, NameStr, BPtr, ByteSize); MacroT1T12[Option1] = 2; }                

  if (MST2==0) { for (n=0; n<MacroSizeM1M12[Option1]; n++) { Mtr1to12[Option2][n] = MacroM1to12[Option1][n]; }    // Copy to Destination Array
                 BPtr = Mtr1to12[Option2];  DoFileBytes(1, MTRname[Option2], BPtr, MacroSizeM1M12[Option1]); }    // Save Destination File

  if (MST2==1) { for (n=0; n<MacroSizeM1M12[Option1]; n++) { Str1to12[Option2][n] = MacroM1to12[Option1][n]; }    // Copy to Destination Array
                 BPtr = Str1to12[Option2];  DoFileBytes(1, STRname[Option2], BPtr, MacroSizeM1M12[Option1]); }    // Save Destination File  

  if (MST2==2) { for (n=0; n<MacroSizeM1M12[Option1]; n++) { Ttr1to12[Option2][n] = MacroM1to12[Option1][n]; }    // Copy to Destination Array
                 BPtr = Ttr1to12[Option2];  DoFileBytes(1, TTRname[Option2], BPtr, MacroSizeM1M12[Option1]); }    // Save Destination File
...
{   int i, n;
    ...
    if (KeyBrdX==3) {if (button==2)   { RemoveMacro(); SendBytesEnd(0);               return; }    // Could remove SendBytesEnd(0); 
                     ...
                     if (button==5)   { CopyMacro(); SendBytesEnd(0);                 return; }    // Could remove SendBytesEnd(0); 
    ...
    if (KeyBrdX<4) 
       {KBDisp[KBDispPos] = b; KBDisp[KBDispPos+1] = ' '; KBDisp[KBDispPos+2] = ' '; 
        ...
        if (Fx) { for (n=0; n<3; n++) {KBDisp[KBDispPos+n] = KeyBrdFxChr[KeyBrdF1F24][n]; }  KeyBrdF1F24++; if (KeyBrdF1F24>23) KeyBrdF1F24=0; } } // Fx true for one MakeStr()

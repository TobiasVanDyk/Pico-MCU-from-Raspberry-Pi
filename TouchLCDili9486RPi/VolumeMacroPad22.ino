...
{"abc",  "def",  "ghi",  "EXE", "jkl", "mno", "pqr", "NXT", "stu", "vwx", "yz ", "ADD",    // ASCII->HID Subtract  dec
 ...
 "\\|?", "\"<>", "\'()", "EXE", ":[]", ";{}", "Fnn", "NXT", "Src", "Dst", "Num", "ADD",    // F1-F12 special code
...
{   int i, n;
    ...
    if (KeyBrdX==3) {if (button==8)   { DoSourceMacro(); status("Source Macro");      return; }  
                     if (button==9)   { DoDestMacro();   status("Destination Macro"); return; }
                     if (button==10)  { DoNumMacro();    return; }
                     if (button==6)   {a = KeyBrdFx[KeyBrdF1F24]; Fx = true;} }  // Xlate F1-F24 - see if (a>0) below where it is used 

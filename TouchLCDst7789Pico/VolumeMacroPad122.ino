...
//                Config Key press Home Volume Mute then Disabled - repaet to enable Volume Mute
...
////////////////////////////////////////////////////////////////////////////////////////////////
const static char Labels[8][12][4] = {"Cut", "Cpy", "Pst", "V+",  "M1",  "M2", "M3",  "Vo",  "M4",  "M5",  "M6",  "V-",  
...
void setup() {
  ...
  */                                 // **[4]**
  ...
  MuteDisable = false;          // VolMute Key active
}
...
void loop() {
  ...
  if ((NowMillis - LastMillis) >= TimePeriod) //test whether the period has elapsed
     {digitalWrite(LCDBackLight, LOW);        // Backlight Off
      ...
      VolMuteCount = 0;                       // VolMuteKey pressed Count = 0 
      ConfigKeyCount = 0;                     // [i][End] sequence will delete files
...
void buttonpress(int button)
{
  ...
  if ((Layout==2) && (button!=8) && (button!=0) && (ConfigKeyCount>0)) 
  ...
  switch(button){
    ...
    case 7: //////////////////////////////////////////////// Volume Mute and change Layouts
      ...
      if (MuteDisable) break;  // No VolumeMute action
  ...
  switch(button){
    ...
    case 5: // CONFIG //////////////////////////////////////////////////
      GetSysInfo(); 
      status("END Delete HOME Mute-OnOff"); 
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  if (MuteDisable) b = b + 4; for (n=0; n<12; n++) for (m=0; m<3; m++) keyLabel[n][m] = Labels[b][n][m]; 
  
  if (MuteDisable) b = b - 4; for (n=0; n<12; n++) keyColor[n] = Colours[b][n];
...
{ //while (!Serial) { delay(100); }
  ...
  Serial.println("VolumeMacroKeys version 122.0");

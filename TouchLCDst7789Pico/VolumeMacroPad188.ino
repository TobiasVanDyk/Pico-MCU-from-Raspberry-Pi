  ...
  if ((KeyBrdByte[0]==0x2A)&&(KeyBrdByte[1]==0x74)&&(KeyBrdByte[2]==0x62))    // *tb*num = blank lcd timemout *tb*0,1-9 = infinite,30,60,90,120,240 sec
   ...
   if ((KeyBrdByte[0]==0x2A)&&(KeyBrdByte[1]==0x6F)&&(KeyBrdByte[2]==0x74))    // *ot*num 
      {t = timePowerOff; timePowerOff=(KeyBrdByte[4]-48)*30;                   // 1 = 30 seconds to 9 = 270 seconds
       ...
       i = 3;                                                                  // 3 digits
       z = timePowerOff;
       while (i!=0) { PowerOffArr[i+12] = (z%10) + 48; z = z/10; i--; };       // modulus (%) and division (/) operations 
       status(PowerOffArr); SendBytesEnd(); return;}  
   if ((KeyBrdByte[0]==0x2A)&&(KeyBrdByte[1]==0x6F)&&(KeyBrdByte[2]==0x84))     // *oT*num 
   ...
   if ((KeyBrdByte[0]==0x2A)&&(KeyBrdByte[1]==0x72)&&(KeyBrdByte[2]==0x74))     // *rt*num 
      {t = timeRestart; timeRestart=(KeyBrdByte[4]-48)*30;                      // 1 = 30 seconds to 9 = 270 seconds
       ...
       i = 3;                                                                   // 3 digits       
       z = timeRestart;
       while (i!=0) { RestartArr[i+12] = (z%10) + 48; z = z/10; i--; };         // modulus (%) and division (/) operations 
       status(RestartArr); SendBytesEnd(); return;}  
   if ((KeyBrdByte[0]==0x2A)&&(KeyBrdByte[1]==0x72)&&(KeyBrdByte[2]==0x84))     // *rT*num 

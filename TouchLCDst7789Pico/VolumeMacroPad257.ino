...
int OptionOS = 0;   // 0 Windows 1 Linux 2 Raspberry Pi OS 3 Apple
char OSName[4][16]   = {"Windows","Linux","Raspberry Pi OS","Apple" };
...
{ unsigned long T, z;  // t = timeclock struct
  ...
  if (KeyBrdByte[1]==0x6F&&KeyBrdByte[2]==0x73)           // *os* operating system toggle Windows/Linux/PiOS on/off
     { OptionOS++; if (OptionOS>2) OptionOS=0; status(OSName[OptionOS]); return true; }
...
void optionsindicators(int Option) {
  ...
  tft.drawString(OSChr[OptionOS], OPT_X+146, OPT_Y-73); 

...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Adapted by T van Dyk Oct 2022 for ST7789 320x240 and Adafruit TinyUSB stack
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool Option1 = false;            // Not assigned as yet - see optionsindicators()
bool Option2 = false;            // Not assigned as yet - see optionsindicators()
...
void setup() {
  ...
  KeyBrdDirect = false;                         // Toggle On/Off with yellow pad in Kbrd mode
  KeyBrd123 = 0;
  PadKeys = false;                              // 5 Small Pads RH side Red Blue Green SkyBlue Gold  
}
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //                 "Hme", "Up",  "PgU", "V+/Del", "<",    "Cfg",  ">",   "Vo/L14",  "End", "Dwn",  "PgD", "V-/Ret"
  // MediaLabel[x][] "Stp", "   ", "   ", "   ",    "Prv",  "   ",  "Nxt", "   ",     "P-P", "   ",  "  ",  "   " 
  // MediaLabel[x][] "|||", "   ", "   ", "   ",    "|<",   "   ",  ">|",  "   ",     ">||", "   ",  "  ",  "   " 
  // keyLabel[x][]     0                              4               6                8                                           
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{ VolDisable  = true;
  ...
  if (LittleFS.format()); 
     {LittleFS.begin();
      ...
      InitCfg(0);
      Serial.println("Config and Macro Files deleted");}
}
////////////////////////////////////////////////
...
{ char *ChrPtr;
  ...
  Serial.println("VolumeMacroPad 202");
...
{ int current = 0;             
  ...
  while (f.available()) { int c = f.read(); if ('0' <= c && c <= '9') { current = current * 10 + (c - '0'); }   } f.close(); 
  if (current<129) DimVal = current; else DimVal = 0;    // 50% max else OFF
}
/////////////////////////////////////////
...
{ uint8_t keycode[6] = { 0 }; // simultaneous keys pressed in here
  ...
  if ((!Fx)&&(KeyBrdByte[0]<0x80))                                             // F1-F12 must be sent with keycode not keyPresssequence via keyPress 
     {for (n = 0; n < KeyBrdByteNum; n++) if (KeyBrdByte[n]<0x80)              // When 1st char is normal alphanumeric all must be <0x80 
          {usb_hid.keyboardPress(HIDKbrd, KeyBrdByte[n]); delay(keydelay);     // Converted via hid.h from ascii to hid
...
{   int i, n;
    ...
    //for (n=0; n<4; n++) {Serial.print(MathHexNum[n]); Serial.print('-'); }    
    status((char *)MathChr);
...
//////////////////////////////////////////////////////////////////////////////////
// Options Indicators status D 1 2 XPos[7] = {0, 0, 78, 156, 0, 78, 156}
//////////////////////////////////////////////////////////////////////////////////
// #define OPT_X 70  // Centred on this - 
// #define OPT_Y 66  // 
//////////////////////////////////////////////////////////////////////////////////
void optionsindicators() {
  ...
  if (Option1)                tft.drawString("1", OPT_X+78, OPT_Y);  else { tft.drawString(" ", OPT_X+78, OPT_Y); }
  if (Option2)                tft.drawString("2", OPT_X+156, OPT_Y); else { tft.drawString(" ", OPT_X+156, OPT_Y); }
  if (KeyFontBold) tft.setFreeFont(&FreeSansBold12pt7b);
              else tft.setFreeFont(&FreeSans12pt7b);   
}
////////////////////////////////////////////////////////////////////////////

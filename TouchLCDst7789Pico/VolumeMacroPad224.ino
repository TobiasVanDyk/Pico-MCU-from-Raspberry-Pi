...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Adapted by T van Dyk Aug to Oct 2022 for ST7789 320x240 and Adafruit TinyUSB stack
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{ "@", "#", "?", "$", "%", "^", "&", ",", ".", "~", "`", "\\", "\'", "\"", "|", ":", ";" };
const static char StarCode[26][5] =    
{ "*bb*", "*cm*", "*ca*", "*de*", "*db*", "*fm*", "*fs*", "*ft*", "*fa*", "*fo*", "*kb*", "*lr*", "*ld*", 
  "*mt*", "*mT*", "*nt*", "*nT*", "*ot*", "*oT*", "*po*", "*po*", "*rt*", "*rT*", "*tb*", "*ul*", "*ua*" };
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
const static char MacroTimerLabel[12][4] = {"R-T",  "Stp",  "O-T", "   ",
                                            "R-t",  "Cfg",  "O-t", "   ",
                                            "R-C",  "Rep",  "O-C", "   " };
                                           
uint16_t MacroTimerColor[12]            = {Green4, Orange, Pink,  Black, 
                                           Green4, Black,  Pink,  Black,  
                                           Green2, Blue,   Red,   Black };                                           
...
bool MacroTimer16  = false;         // One of the Timers1-4 has been set Once-of or Repeat Macro by time or by clock (need PicoW?)
...
void loop() {
  ...
  if (MacroTimer16) CheckMacroTimers();                               // Check Timers1-4 Oneshot and repeating macros
...
{ unsigned long TimeNow = millis();  
  ...
  if (MacroTimer1) { if ((TimeNow - TimeRepeatPrev) >= TimeRepeat) 
                        { TimeRepeatPrev = TimeNow; LayoutPrev = Layout; Layout = MacroTimerArr1[0]; 
                          MacroKeys(MacroTimerArr1[1]); Layout = LayoutPrev; optionsindicators();} }
                   
  if (Macrotimer2) { if ((TimeNow - timeRepeatPrev) >= timeRepeat) 
                        { timeRepeatPrev = TimeNow; LayoutPrev = Layout; Layout = MacroTimerArr2[0]; 
                          MacroKeys(MacroTimerArr2[1]); Layout = LayoutPrev; optionsindicators();} }
    
  if (MacroTimer3) { if ((TimeNow - TimeOnceofPrev) >= TimeOnceof) 
                        { MacroTimer3 = false; LayoutPrev = Layout; Layout = MacroTimerArr3[0]; 
                          MacroKeys(MacroTimerArr3[1]); Layout = LayoutPrev; optionsindicators();} }
                     
  if (Macrotimer4) { if ((TimeNow - timeOnceofPrev) >= timeOnceof) 
                        { Macrotimer4 = false; LayoutPrev = Layout; Layout = MacroTimerArr4[0]; 
                          MacroKeys(MacroTimerArr4[1]); Layout = LayoutPrev; optionsindicators();} }
                               
  MacroTimer16 = Macrotimer4 || MacroTimer3 || Macrotimer2 || MacroTimer1; 
...
////////////////////////////////////////////////////////////////////////
bool MacroLink (byte c)  // Only 5 for the time being repeat is planned
////////////////////////////////////////////////////////////////////////
{byte n;
...
/////////////////////////////
{
  ...
  // Macro Timers must either be executed (buttons 1 or 0,2,4,6,8,9,10) or cancelled (button 5 or 3,7,11)  
  if (MacroTimerDisp) { if (button==3||button==7||button==11) { if (MacroTimer16)  status("Macro Timer running in background");  
                                                                if (!MacroTimer16) status("No Macro Timer running");
  ...
  if (PowerKeys) {if (button==3||button==7||button==11) {status("Power Keys cancelled"); PowerKeys=false; ConfigButtons(1); return;} }
  ...
  switch(button){
    case 0: // HOME - Media Previous //////////////////////////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { MacroTimer1 = !MacroTimer1; MacroTimer16 = MacroTimer16 || MacroTimer1;
                        if (MacroTimer1) { TimeRepeatPrev = millis(); status("Repeat Macro Timer (T) started"); }
                                                                 else status("Repeat Macro Timer (T) stopped");
                        ConfigButtons(1); optionsindicators(); break; }
    ...
    case 1: // ARROW UP ////////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { MacroTimer6 = MacroTimer5 = Macrotimer4 = MacroTimer3 = Macrotimer2 = MacroTimer1 = MacroTimerDisp = MacroTimer16 = false;
                        status("All Macro Timers stopped");
                        ConfigButtons(1); optionsindicators(); break; }
    ...
    case 2: // PAGE UP /////////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { MacroTimer3 = !MacroTimer3; MacroTimer16 = MacroTimer16 || MacroTimer3;
                        if (MacroTimer3){ TimeOnceofPrev = millis(); status("Oneshot Macro Timer (T) started"); }
                                                                else status("Oneshot Macro Timer (T) stopped");
                        ConfigButtons(1); optionsindicators(); break; }
    ...
    case 4: // ARROW LEFT //////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { Macrotimer2 = !Macrotimer2; MacroTimer16 = MacroTimer16 || Macrotimer2;
                        if (Macrotimer2) { timeRepeatPrev = millis(); status("Repeat Macro Timer (t) started"); } 
                                                                 else status("Repeat Macro Timer (t) stopped");
                        ConfigButtons(1); optionsindicators(); break; }
    ...
    case 6: // ARROW RIGHT /////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { Macrotimer4 = !Macrotimer4; MacroTimer16 = MacroTimer16 || Macrotimer4;
                        if (Macrotimer4) { timeOnceofPrev = millis(); status("Oneshot Macro Timer (t) started"); }
                                                                 else status("Oneshot Macro Timer (t) stopped");
                        ConfigButtons(1); optionsindicators(); break; }
    ...
    case 8: // END //////////////////////////////////////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { // MacroTimer5 = !MacroTimer5; // MacroTimer16 = MacroTimer16 || MacroTimer5;
                        if (MacroTimer5) status("Repeat Macro Clock Time started"); else status("Repeat Macro Clock Time stopped");
                        ConfigButtons(1); break; }
    ...
    case 10: // PAGE DOWN //////////////////////////////////////////////
      ...
      if (MacroTimerDisp) { // MacroTimer6 = !MacroTimer6; // MacroTimer16 = MacroTimer16 || MacroTimer6;
                        if (MacroTimer6) status("Oneshot Macro Clock Time started"); else status("Oneshot Macro Clock Time stopped");
                        ConfigButtons(1); optionsindicators(); break; }
...
{ char *ChrPtr;
  ...
  if (SaveReadLinks) SaveLinks();  // Restored by ReadLinks() in InitCfg
...
{ byte a, b, c, n;
  ...
  char XLat3[4][4]  = {"R-T","R-t","O-T","O-t" };
  b = 0; // b = 1,2,3,4 = TimerName[0-3] = Timer type Repeat T Repeat t Once T Once t
  if (KeyBrdByteNum>0) { a = KeyBrdByte[0]; b = a - 48; }  // add 1-4 and a ='1'-'4' b=1-4
  
  //for (n=0; n<4; n++) TimerName[n][0] = 0x00;
  ...
  if (b>4||b==0||KeyBrdByteNum==0) { status("Need 1-4 option added"); return false; } 
...
{   int i, n;
    ...
    if (KeyBrdX==2) {if (button==10) {a = KeyBrdFx[KeyBrdF1F24]; Fx = true;}             // Xlate F1-F24 - see if (a>0) below where it is used 
                     ...
                     if (button==6)  { for (n=0; n<4; n++) { b = StarCode[StarNum][n]; KeyBrdByte[n] = b; KBDisp[n] = b; } 
                                       ...
                                       StarNum++; if (StarNum==25) StarNum = 0;      return; } }
...
void optionsindicators() {
  ...
  if (!Kbrd) if (MacroTimer16) tft.drawString("T", OPT_X, OPT_Y);         else { tft.drawString(" ", OPT_X, OPT_Y);      } // Must come after the previous line

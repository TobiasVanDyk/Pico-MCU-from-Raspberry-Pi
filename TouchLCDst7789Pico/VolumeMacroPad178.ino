...
///////////////////////////////////////////////////////////////////////////////
uint16_t calData[5] = {437, 3456, 267, 3508, 5};    // **[1]**
...
/////////////////////////////////////////////////////////////////////////////////////////////
// Layout 3 small Config Buttons N K M (Numeric Keyboard Media) between 1st and 2nd row
/////////////////////////////////////////////////////////////////////////////////////////////
#define PAD_W 14          // Key width  PAD_X is middle -> Pad X is from 317 to 303 
#define PAD_H 24          // Key height
#define PAD_SPACING_Y 74  // Next left 
#define PAD_X 310         // config buttons left side
...
{"alpha", "delta",   "eta",   "0", "kappa",              "nu",          "pi",            "0 ","sigma2",         "phi",          "omega",          "0",
 ...
 "Theta", "Pi",     "Psi",    "0", "disjoint-union",     "element_of",  "subset-of",     "0", "subset-or-equal","plusminus",    "infinity",       "0",
 "sum",  "volume_integral","triple_integral", "0", "fourth_root", "n_ary_prod",    "notequal",     "0", "identicalto",   "combining_tilde", "combining_dot", "0",
 "arrow","integral",       "line_integral",   "0", "square_root", "partial_deriv", "almostequal",  "0", "lessorequal",   "combining_bar",   "combining_2dot", "0",
 "nabla","double_integral","surface_integral","0", "cube_root",   "prime",         "proportional", "0", "greaterorequal","combining_arrow", "double_prime"    "0" };
 
// 3 Small Config Buttons between 1 st and 2bd row Red Blue Green
char padLabel[3][4] = {"", "", ""}; 
uint16_t padColor[12]   = {Black, Black, Black, Red, Blue, Green4, Black, Black, Black, Black, Black, Black};
uint16_t padOutline[12] = {Black, Black, Black, White, White, White, Black, Black, Black, Black, Black, Black};
...
bool PadKeys = false;            // 3 Small Config Buttons on RH side Red Blue Green
int  PadKeyRBG = 0;              // Red Blue Green = 0 1 2 
bool PadKeyR = false;
bool PadKeyB = false;
bool PadKeyG = false;
...
// Create 12 keys + 3 config buttons for keypad
...
void setup() {
  ...
  */                                 // **[4]**
  ...
  KeyBrdDirect = false; 
  KeyBrd123 = 0;
  PadKeys = false;                              // 3 Small Config Buttons on RH side Red Blue Green
  PadKeyRBG = 0;                                // Red Blue Green = 0 1 2
}
...
void loop() {
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  for (uint8_t b = 0; b < 15; b++) { if (pressed && key[b].contains(t_x, t_y)) key[b].press(true); else key[b].press(false); }                    
  ...
  for (uint8_t b = 0; b < 15; b++) { if (key[b].justReleased()) {key[b].drawButton(false); RepLast = RepNow; }                      // draw normal - code at release time
...
void buttonpress(int button)
{
  ...
  //////////////////////////////////////////////////
  // 3 Config Pads Red Blue Green button > 11
  // Handle 1st then return - as if button = 0-11
  //////////////////////////////////////////////////
  if (button>11) 
     {PadKeys = true; 
      if (button==12) { PadKeyR = (!PadKeyR); PadKeyB = false;                        // PadKeyRBG 0 Toggle Numeric Pad 
                        if (Math) SendBytesEnd();                                     // This sets Math = false;
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  for (row = 0 + rowcount; row < rows; row++) {   // 3 rows 0,1,2 => b = 0+(0*4)=0 1+(0*4)=1 ... 0+(1*4)=4 ....
   ...
   b = Layout - 1;                  // reset b to 0 - 3 
   for (row = 0; row < 3; row++) {PadB = padOutline[row+(b*3)]; PadC = padColor[row+(b*3)]; // row = 0,1,2 + (0 1 2 3 = Layout-1) * 3
        //                          x-middle    y-middle                 width height outline fill  text
        //                            308    36 + 0,1,2 * 74 = 0,110,184   10    20             

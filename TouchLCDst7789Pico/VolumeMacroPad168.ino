...
sequence type (200 chars max). Press [Up] after sending the macros to the PC to assign them to [M1] or [M2].
...
void setup() {
  ...
  if (LittleFS.exists("KeyBrdMacro1"))     Macro1      = 2;     else Macro1      = 0;       // KeyBrd entered macros seq
  if (LittleFS.exists("KeyBrdMacro2"))     Macro2      = 2;     else Macro2      = 0;       // KeyBrd entered macros par
...
{ int n = 0;
  ...
  if (Found){ if ((LayerAB==1)&&(a>0)) c = a + 5; else c = a - 1;    // S1-S6=>S7-S12 T1-T6=>T7=T12 M1-M6=>M7=M12
              ...
              if (Layout==1) {if (a>0) {ByteOK = true; NameStr = MTRname[c]; BytePtr = Mtr1to12[c]; 
                                        ASize = ARRAY_SIZE(Mtr1to12[c]);}                               // Keys M1-M12
                                  else {ByteOK = true; return; }   }                                   // Key [M4] non-ASCII in RecBytes[] 
   ...
   if ((Found)&&(ByteOK)) {DoFileBytes(ByteOK, NameStr, BytePtr, ASize); } // Save the numbers 
...
void buttonpress(int button)
{
      ...
      if (Layout==1) 
         {if (Macro1==2) 
             { for (n=0;  n<Macro1Size; n++) 
              ...
              // const byte arraySize = sizeof(Mtr1to12[c]) / sizeof(Mtr1to12[c][0]);
  ...
  switch(button){
    ...
    case 11: // V- or Enter or Add /////////////////////////////////////////////////////////////////////////////////////////////////////
      if (Kbrd) {if (!KBrdActive) {status("Press char/modifier first"); break;} // No char key or modifier keys has been pressed at least once
...
{ char *ChrPtr;
  ...
  for (n=0; n<12; n++) 
      {NameStr = MTRname[n]; BPtr = Mtr1to12[n]; ASize = ARRAY_SIZE(Mtr1to12[n]);
       if (LittleFS.exists(NameStr)) DoFileBytes(ByteOK, NameStr, BPtr, ASize); }

  NameStr = "KeyBrdMacro1"; BPtr = Mtr1; ASize = ARRAY_SIZE(Mtr1);
       if (LittleFS.exists(NameStr)) DoFileBytes(ByteOK, NameStr, BPtr, ASize); 

  NameStr = "KeyBrdMacro2"; BPtr = Mtr2; ASize = ARRAY_SIZE(Mtr2);
       if (LittleFS.exists(NameStr)) DoFileBytes(ByteOK, NameStr, BPtr, ASize); 
...
//////////////////////////////////////////////////////////////////
void DoFileBytes(bool DoWrite, String STRf,  byte *BytePtr, uint16_t ByteArrayLen)
{ uint16_t ByteLen;
  int n;
  byte b;
...
{ uint16_t StrLen;
  ...
  if (DoWrite)  // write data
     {File f = LittleFS.open(STRf, "w");
      StrLen = strlen(ChrPtr);               // ok to do pointer size if strings?
...
{  char *ChrPtr;
  ...
  Serial.println("VolumeMacroKeys version 166.0");

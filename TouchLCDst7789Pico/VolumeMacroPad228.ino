...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  if ((Media)&&(b==1)&&(ConfigKeyCount==0)) {
      if (!ToneControls) {for (m=0; m<4; m++) {n = mR[m]; strcpy(keyLabel[n], MediaLabel[n]); }
...
{ int m, n;        
  ...
  n = 0;
  if (MST1==0) { if (LittleFS.exists(MTRname[Option1])) { status("[M] already saved"); return false; }
  ...
  if (MST1==1)  { if (LittleFS.exists(STRname[Option1])) { status("[S] already saved"); return false; }
  ...
  if (MST1==2) { if (LittleFS.exists(TTRname[Option1])) { status("[T] already saved"); return false; }

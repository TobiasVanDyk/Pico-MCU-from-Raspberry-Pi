...
// Two sets of 12 button colours - use the colours defined in TFT_eSPI.h const static uint16_t Colours[2][12]
uint16_t Colours[2][12] =       {TFT_BLUE, TFT_RED, TFT_ORANGE, TFT_DARKCYAN, 
                                 ...
                                 TFT_BLUE, TFT_GOLD, TFT_RED, TFT_BLUE, 
                                 TFT_GOLD, TFT_BLACK, TFT_GOLD, TFT_RED, 
                                 TFT_BLUE, TFT_GOLD, TFT_RED, TFT_ORANGE  };
                             
uint16_t keyColor[12] = {};

uint16_t BackgroundColor[2] = {TFT_BLACK, TFT_NAVY}; 
...
void hid_report_callback(uint8_t report_id, hid_report_type_t report_type, uint8_t const* buffer, uint16_t bufsize)
{
  ...
  **********************************************************/
  ...
  if (CapsLock)                    { Colours[0][8] = TFT_GREEN;  // Use unique colour
                                     Colours[1][8] = TFT_GREEN;  // Use unique colour
                                     ConfigButtons(); }
  ...
  if (NumLock)                     { Colours[0][9] = TFT_GREEN;  // Use unique colour
                                     Colours[1][9] = TFT_GREEN;  // Use unique colour
                                     ConfigButtons(); }
  if (!NumLock)                    { Colours[0][9] = TFT_LIGHTGREY;  // Use a unique colour
                                     Colours[1][9] = TFT_GOLD;        // Use a unique colour
                                     ConfigButtons(); }  }            // Draw Buttons and Labels
                                    
  if (ScrollLock!=ScrollLockPrev){                                     
  if (ScrollLock)                  { Colours[0][10] = TFT_GREEN;  // Use unique colour
                                     Colours[1][10] = TFT_GREEN;  // Use unique colour
                                     ConfigButtons(); }

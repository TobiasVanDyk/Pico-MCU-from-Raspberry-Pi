...
(*) Must still implement F1-F12, and simultaneous keys other than modifiers + A-Z
***********************************************************************************************************************************/
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const static char KbrdLabel[5][12][4] = 
{"abc",  "def",  "ghi",  "ESC", "jkl", "mno", "pqr", "NXT", "stu", "vwx", "yz ", "ADD",
 "ABC",  "DEF",  "GHI",  "esc", "JKL", "MNO", "PQR", "nxt", "STU", "VWX", "YZ_", "add",
 "012",  "345",  "678",  "ESC", "9+-", "=*/", "!@#", "NXT", "$%^", "&,.", "~` ", "ADD",    
...
char KeyBrdStr[200]={};        // Used for display of non-printing chars - SHF SHL SHR 
int  PrevButton = 0;           // 
char KBDisp[200]={};
int  KBDispPos = 0;
  ...
  switch(button){
    ...
    case 3: // V+ or Delete or Escape(Keybrd) //////////////////////////////////////
      if (Kbrd) {Kbrd = false;  status((char *)KBDisp);
                 if ((KeyBrdX==4) && (KeyBrdByteNum>0)) {SendBytes(); status("Macro sent");} 
    ...
    case 11: // V- or Enter or Add /////////////////////////////////////////////////////////////////////////////////////////////////////
      if (Kbrd) {if (KeyBrdX==4) {KBDispPos = KBDispPos + 2; } // Added char/num in byte and display arrays point to next space in array
...
{ int n;
  byte keycode[6] = {0};  // simultaneous keys pressed in here
  byte a, b;              // a = b-61 to convert ACII to HID_KEY_A-X etc 
  
  if (KeyBrdByte[0]<0x80)  // Normal A-X a-x 0-9 characters
     {for (n = 0; n < KeyBrdByteNum; n++) 
          {usb_hid.keyboardPress(RID_KEYBOARD, KeyBrdByte[n]); delay(keydelay);    // does this auto convert HID-normal keys to ASCII?
           usb_hid.keyboardRelease(RID_KEYBOARD);              delay(keydelay); }
      status("Macro Type 1"); }
          
  if ((KeyBrdByte[0]>0x7F) && (KeyBrdByteNum<6))
     {for (n = 0; n < KeyBrdByteNum; n++) {b = KeyBrdByte[n]; if ((b>=0x40) && (b<=0x5B)) b = b - 61; keycode[n] = b; }
          usb_hid.keyboardReport(RID_KEYBOARD, 0, keycode); delay(keydelay);
          usb_hid.keyboardRelease(RID_KEYBOARD);            delay(keydelay);  }
      /*    
      for (n = 0; n < KeyBrdByteNum; n++) 
  ...
  for (n = 0; n < 200; n++) { KeyBrdByte[n] = 0x00; KeyBrdStr[n] = '\0';  KBDisp[n] = '\0'; }
  Kbrd = false;
}
...
{   int i, n;
    ...
    //Serial.print(KeyBrdByteNum);
    //Serial.print(" number HEX values ");

    b = KeyBrdByte[KeyBrdByteNum];
    if ((b>=0x40) && (b<=0x5B)) byte a = b - 61;
    ...
    */
    ...
    n = 0;
    if (KeyBrdX<4) KBDisp[KBDispPos] = b;

...
const static char PowerKeysLabel[3][4] = {"Log", "Off", "Rst"};
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////
void DoPowerKeys (char Cmd, bool Menu) 
///////////////////////////////////////
{ uint8_t keycode[6] = { 0 };     // simultaneous keys pressed in here
  char ShutDwn[3][45] = { "C:\\Windows\\System32\\shutdown.exe /l",
                          "C:\\Windows\\System32\\shutdown.exe /s /t 0",
                          "C:\\Windows\\System32\\shutdown.exe /r /t 0"};                      
  int n, s = 0;
  
  PowerKeys = false; // Must do this before the actions below
  status(" ");
  ConfigButtons(1);
  
  if (Menu) { keycode[0] = HID_KEY_GUI_LEFT; // or use HID_KEY_GUI_LEFT 
  ...
  if (Cmd=='r') s = 2; if (Cmd=='u') s = 1; // s=0 default
  for (n=0;  n<strlen(ShutDwn[s]); n++) 
                           {usb_hid.keyboardPress(HIDKbrd, ShutDwn[s][n]); delay(keydelay2);
                            usb_hid.keyboardRelease(HIDKbrd);              delay(keydelay2); } 
...
/////////////////////////////
{
  ...
  // Power Keys must either be executed (buttons 8,9,10) or cancelled (button 5)
  if (PowerKeys) {if ((button!=5)&&(button!=8)&&(button!=9)&&(button!=10)) {status("Power Keys cancelled"); PowerKeys=false; ConfigButtons(1); return;} }
  ...
  switch(button){
    ...
    case 8: // END //////////////////////////////////////////////////////////////////////////////
      if (PowerKeys) { DoPowerKeys ('i', PowerKeysMenu); break;} 
    ...
    case 9: // ARROW DOWN //////////////////////////////////////////////
      ...
      if (ConfigKeyCount==1) {ConfigKeyCount--; PowerKeys = !PowerKeys;  // = true; // works differently but ok
          if (PowerKeys) status("[Log]off Power[Off] [Res]tart"); else status(" ");
          ConfigButtons(1); break;}   
             
      if (PowerKeys) { DoPowerKeys ('u', PowerKeysMenu); break; }
    ...
    case 10: // PAGE DOWN //////////////////////////////////////////////
      if (PowerKeys) { DoPowerKeys ('r', PowerKeysMenu); break; }
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // PowerKeysLabe[3][4] {"Log", "Off", Rst};
  // Bottom row on Cfg Layout 3 Logoff Off Restart
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if ((PowerKeys)&&(b==1)&&(ConfigKeyCount==0)) { for (m=0; m<3; m++)          // Bottom row on Cfg Layout 3 Logoff Off Restart
                 {keyLabel[8][m]  =  PowerKeysLabel[0][m];   keyLabel[9][m]  = PowerKeysLabel[1][m];  keyLabel[10][m] = PowerKeysLabel[2][m]; }
                 keyColor[8] = Blue; keyColor[9] = Red; keyColor[10] = Green4; }              

...
{ "*bb*", "*ca*", "*cm*", "*ct*", "*de*", "*db*", "*fm*", "*fs*", "*ft*", "*fa*", "*fo*", "*kb*", "*lr*", "*ld*", 
...
{ int n = 0;
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (aTime) {if (RecBytes[1]==0x2d) alarm.year  = -1; else { alarm.year  = 2000 + (RecBytes[1]-48)*10 + (RecBytes[2]-48);  }
              if (RecBytes[3]==0x2d) alarm.month = -1; else { alarm.month = (RecBytes[3]-48)*10 + (RecBytes[4]-48); }
              if (RecBytes[5]==0x2d) alarm.day   = -1; else { alarm.day   = (RecBytes[5]-48)*10 + (RecBytes[6]-48); }
              if (RecBytes[7]==0x2d) alarm.dotw  = -1; else { alarm.dotw  = (RecBytes[7]-48); }
              if (RecBytes[8]==0x2d) alarm.hour  = -1; else { alarm.hour  = (RecBytes[8]-48)*10 + (RecBytes[9]-48); }       
              if (RecBytes[10]==0x2d) alarm.min  = -1; else { alarm.min   = (RecBytes[10]-48)*10 + (RecBytes[11]-48); } alarm.sec = 0; 
              // rtc_set_alarm(&alarm, &alarm_callback); delay(keydelay2); 
              status("Macro Timer received use [R-C][O-C]]"); return; } 

  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // This is still the same type of alarm timer as ATime
  // Will be changed to Clock Time countdown timer: add_alarm_in_ms(2000, alarm_callback, NULL, false);
  //                                                add_repeating_timer_ms(500, repeating_timer_callback, NULL, &timer);
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (wTime) {if (RecBytes[1]==0x2d) timer.year  = -1; else { timer.year  = 2000 + (RecBytes[1]-48)*10 + (RecBytes[2]-48);  }
              if (RecBytes[3]==0x2d) timer.month = -1; else { timer.month = (RecBytes[3]-48)*10 + (RecBytes[4]-48); }
              if (RecBytes[5]==0x2d) timer.day   = -1; else { timer.day   = (RecBytes[5]-48)*10 + (RecBytes[6]-48); }
              if (RecBytes[7]==0x2d) timer.dotw  = -1; else { timer.dotw  = (RecBytes[7]-48); }
              if (RecBytes[8]==0x2d) timer.hour  = -1; else { timer.hour  = (RecBytes[8]-48)*10 + (RecBytes[9]-48); }       
              if (RecBytes[10]==0x2d) timer.min  = -1; else { timer.min   = (RecBytes[10]-48)*10 + (RecBytes[11]-48); } timer.sec = 0; 
              // rtc_set_alarm(&timer, &timer_callback); delay(keydelay2);
              status("Macro Timer received use [RcT][OcT]"); return; }       

  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (pTime) {if (RecBytes[1]==0x2d) power.year  = -1; else { power.year  = 2000 + (RecBytes[1]-48)*10 + (RecBytes[2]-48);  }
              if (RecBytes[3]==0x2d) power.month = -1; else { power.month = (RecBytes[3]-48)*10 + (RecBytes[4]-48); }
              if (RecBytes[5]==0x2d) power.day   = -1; else { power.day   = (RecBytes[5]-48)*10 + (RecBytes[6]-48); }
              if (RecBytes[7]==0x2d) power.dotw  = -1; else { power.dotw  = (RecBytes[7]-48); }
              if (RecBytes[8]==0x2d) power.hour  = -1; else { power.hour  = (RecBytes[8]-48)*10 + (RecBytes[9]-48); }       
              if (RecBytes[10]==0x2d) power.min  = -1; else { power.min   = (RecBytes[10]-48)*10 + (RecBytes[11]-48); } power.sec = 0;
              // rtc_set_alarm(&power, &power_callback); delay(keydelay2);             
              status("Power Timer received use [O-C][R-C]"); return; }              
...
/////////////////////////////
{
  ...
  switch(button){
    ...
    case 5: // CONFIG //////////////////////////////////////////////////
      ...
      if (ConfigKeyCount==1) { status("Config and Files Saved"); 
                               if (TimeSet) { delay(1000); status(datetime_str); } ConfigKeyCount = 0; }
...
{ char *ChrPtr;
  ...
  Serial.println("Version: VolumeMacro 49 GPL3 Nov 2022");

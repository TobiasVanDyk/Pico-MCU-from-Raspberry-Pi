...
//                (3) Bodmer-Adafruit Keypad Keypad_320x240.ino
...
//
// Adapted by Tobias van Dyk August to June 2023 for the ST7789 320x240 LCD and Adafruit 
// TinyUSB stack and the remarkably versatile and agile human opposable thumb(s).
/////////////////////////////////////////////////////////////////////////////////////////////////////// 
/************************************************************************************
...
int  SDNum = 1;         // 1,2,3 - Select between 3 groups of 24 files on SDCard according to its GB capacity. SDnum=0 => SDCard file read disabled
...
/////////////////////////////////////////////////////////////////////////////////////////////
#define PAD_W 22          // Key width  PAD_X is middle -> Pad X is from 317 to 303 
#define PAD_H 36          // Key height
#define PAD_SPACING_Y 40  // was 74 for 3 pads // Next below 
#define PAD_X 304         // config buttons right side
#define PAD_Y 26          // config buttons top
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
#define OPT_X 70           // Centred on this - Space in RH side about 16 pixels
#define OPT_Y 205          // 
////////////////////////////////////////////////////////////////////////////////
...
{ "@", "#", "?", "$", "%", "^", "&", ",", ".", "~", "`", "\\", "\'", "\"", "|", ":", ";" };
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const static int StarCodesMax = 57; // StarCodes Count 15+15+15+12
...
{ "*am*", "*as*", "*at*", "*bb*", "*br*", "*ca*", "*cm*", "*ct*", "*db*", "*de*", "*e0*", "*e1*", "*e2*", "*e3*", "*e4*", 
  ...
  "*nT*", "*os*", "*ot*", "*oT*", "*po*", "*rt*", "*rT*", "*sa*", "*sd*", "*se*", "*sm*", "*ss*", "*st*", "*ta*", "*tb*",
  "*tp*", "*tt*", "*tw*", "*ua*", "*ul*", "*0R*", "*x1*", "*x2*", "*x3*", "*x4*", "*x5*", "*x6*" };
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//       RColour[12] = {Black, Brown, Red, Orange, Yellow, Green2, Blue, Purple, DGrey, White1, Gold, Silver };  // 4 band - 3 band colour
...
{ SPI1.setSCK(pinSdClk);
  ...
  if (SaveLayout[0]>0) Layout = SaveLayout[0]; 
      else Layout = 2;                          // if SaveLayout > 0 Layout 1 to 4 else default Layout 2 (Cfg)
...
{ if (Restore) { status(" ") ;                                                   }                            
  ...
  if (MouseK) status("MouseLMR-BF ScrollUD CursorUDLR"); else status(" ");
}
...
/////////////////////////////
{
  ...
  ///////////////////////////////////////////////////////////////////////////////
  // 5 Config Pads Red DarkBlue SkyBlue Gray Green button > 11
  ...
  if ((button>11)&&(Layout!=2))   // 5 Navigation Pads Purple DarkGray LightGray Pink Purple button > 11                                                    
     {if (button==16) { if (LayerAD<3) LayerAD++; else LayerAD = 0;}                  
      if (button==12) { if (LayerAD>0) LayerAD--; else LayerAD = 3;} 
      if (button==16||button==12) {PadKeys = true; LayerADLetter[0] = 97 + LayerAD; optionsindicators(0); ConfigMacroButtons(); return; }
    ...
    case 4: /////////// 12 keys handled here: M1 M7 M13 M19 - S1 S7 S13 S19 - T1 T7 T13 T19 - select by [Layout=1,3,4 and LayerAD=0,1,2,3        
         ...
         if (Layout==1)                                                            // Keys M1 M7 M13 M19
            { if (!LayerAxD) { if (LinkM[c]==0) { if (MacroKeys(c, 0)) break; }    // If not "X" and macro defined then do this
              ...
              keycode[0] = Windows1[c]; keycode[1] = Windows2[c]; keycode[2] = Windows3[c]; keycode[3] = Windows4[c];
                                                                                                           
              usb_hid.keyboardReport(HIDKbrd, 0, keycode); delay(keydelay);
              usb_hid.keyboardRelease(HIDKbrd);            delay(keydelay2); 
              status(Labels[LayerAD][Layout-1][button]); }  break; // Layout=1 or default
    ...
    case 5: /////////// 12 keys handled here: M2 M8 M14 M20 - S2 S8 S14 S20 - T2 T8 T14 T20 - select by [Layout=1,3,4 and LayerAD=0,1,2,3 
        ...
        if (Layout==1)                                                               // Keys M1 M7 M13 M19
            { if (!LayerAxD) { if (LinkM[c]==0) { if (MacroKeys(c, 0)) break; }
              ...
              usb_hid.keyboardReport(HIDKbrd, 0, keycode); delay(keydelay);
              usb_hid.keyboardRelease(HIDKbrd);            delay(keydelay2); 
              status(Labels[LayerAD][Layout-1][button]); }  break; // Layout=1
    ...
    case 6: /////////// 12 keys handled here: M3 M9 M15 M21 - S3 S9 S15 S21 - T3 T9 T15 T21 - select by [Layout=1,3,4 and LayerAD=0,1,2,3 
        ...
        if (Layout==1) 
            { if (!LayerAxD) { if (LinkM[c]==0) { if (MacroKeys(c, 0)) break; }
              ...
              usb_hid.keyboardReport(HIDKbrd, 0, keycode); delay(keydelay);
              usb_hid.keyboardRelease(HIDKbrd);            delay(keydelay2); 
              status(Labels[LayerAD][Layout-1][button]);   break; }  break; // Layout=1
    ...
    case 9: /////////// 12 keys handled here: M5 M11 M17 M23 - S5 S11 S17 S23 - T5 T11 T17 T23 - select by [Layout=1,3,4 and LayerAD=0,1,2,3
        ...
        if (Layout==1) 
            { if (!LayerAxD) { if (LinkM[c]==0) { if (MacroKeys(c, 0)) break; }
              ...
              usb_hid.keyboardReport(HIDKbrd, 0, keycode); delay(keydelay);
              usb_hid.keyboardRelease(HIDKbrd);            delay(keydelay2); 
              status(Labels[LayerAD][Layout-1][button]);  }    // Layout=1
          break; 
  ...
  switch(button){
    ...
    case 1: // ARROW UP ////////////////////////////////////////////////
      ...
      if (ConfigKeyCount==1) {ConfigKeyCount--;
                              BsDNum++; if (BsDNum>BSDMax) BsDNum=0;  // Delete BackSpace Tab AltTab Insert Esc PScr etc
    ...
    case 4: // ARROW LEFT //////////////////////////////////////////////
      ...
      if (ConfigKeyCount==1) {ConfigKeyCount--; LayerAD++; if (LayerAD==4) LayerAD = 0; LayerADLetter[0] = 97 + LayerAD;
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if (MouseK) {for (m=0; m<12; m++) {strcpy(keyLabel[m], MouseKeysLabel[m]); keyColor[m] = MouseKeyColour[m]; }; goto KeyBrdsDone; }     // even thrice    
...
{ char *ChrPtr;
  ...
  Serial.println("Version: VolumeMacro262 GPL3 Tobias van Dyk June 2023");
...
void optionsindicators(int Option) {
  ...
  if (!LayerAxD) LayerADLetter[0] = 97 + LayerAD; else LayerADLetter[0] = 'x';
  tft.drawString(LayerADLetter, OPT_X+146, OPT_Y-143);

  //tft.drawString(OSChr[OptionOS[0]], OPT_X+146, OPT_Y-73); 
  tft.drawString(OSChr[OptionOS[0]], OPT_X+73, OPT_Y-143); 

  if (!CheckSerial) tft.setTextColor(Orange, Black); else tft.setTextColor(Green, Black);
  tft.drawString("s", OPT_X, OPT_Y-143); 

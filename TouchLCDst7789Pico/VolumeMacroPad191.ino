...
{ uint8_t keycode[6] = { 0 };     // simultaneous keys pressed in here                  
  ...
  if ((Menu)&&(s>0)) { keycode[0] = HID_KEY_GUI_LEFT; // or use HID_KEY_GUI_LEFT 
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  for (row = 0 + rowcount; row < rows; row++) {   // 3 rows 0,1,2 => b = 0+(0*4)=0 1+(0*4)=1 ... 0+(1*4)=4 ....
   ...
   b = Layout - 1;                  // reset b to 0 - 3 
   //                                     0+0*5=0   0+1*5=5 0+2*5=10 0+3*3=9  
   //                                     1+0*5=1   1+1*5=6 1+2*5=11
   //                                     2     2   2     7 2     12
   //                                     3     3   3     8 3     13
   //                                     4+0*5=4   4+1*5=9 4+2*5=14 4+3*5=19
...
{ char *ChrPtr;
  ...
  Serial.println("VolumeMacroKeys version 190.0");
...
void ReadLCDBlank() 
//////////////////////
{ int current = 0;  // https://stackoverflow.com/questions/62816850/arduino-read-integer-from-txt
  File f = LittleFS.open("LCDBlankTime", "r");  
  while (f.available()) { int c = f.read(); if ('0' <= c && c <= '9') { current = current * 10 + (c - '0'); }   } f.close(); 
  if (current>10000) TimePeriod = current; else TimePeriod = 30000;
}
...
{ char PArray[4][10] = {"tRestart", "TRestart", "tPowerOff", "TPowerOff"};
  ...
  //                        status((char *)MathChr); status((char *)KBDisp); status(BlankArr); status(RPArr[Option]);
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////
...
{ LittleFS.begin();
  ...
  if (LittleFS.exists(CalFile))   // check if calibration file exists and size is correct
     {if ( DoCal)                 // Delete if we want to re-calibrate
         { LittleFS.remove(CalFile);}
      else {File f = LittleFS.open(CalFile, "r");
            if (f) {if (f.readBytes((char *)calData, 14) == 14) calDataOK = 1; 
                        f.close(); }  }  }
  ...
  if (calDataOK && !DoCal)         // calibration data valid
     {tft.setTouch(calData);}    
            ...
            if (f) { f.write((const unsigned char *)calData, 14);
                     f.close();
         }
    }
  }

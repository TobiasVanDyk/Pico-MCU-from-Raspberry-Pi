...
[Send Text ] [Admin CMD Prompt] [TaskManager ] [Vol Dwn]         [M4]  [M5]  [M6]  [V-][Enter]
...
// 5. Observe (display or serial) the first 4 of 5 calibration points
...
// Modified by Caps-Scroll-Num Lock Keys 
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////
const static char Labels[16][12][4] = {"Cut", "Cpy", "Pst", "V+",  "M1",  "M2", "M3",  "Vo",  "M4",  "M5",  "M6",  "V-",  // L1 Mute ON Vol=ON
...
char LayerStr[4][3] = {"L1","L2","L3","L4"};   //Start with 7 followed by the startup layer 1 to 4 L1-L4
...
void setup() {
  ...
  */
  ...
  InitStr(); // Copy str# to Str# and Ttr# if no saved file else read Str# Ttr# from file system
         ...
         if ((a<7)&&(!Found)&&(n==0)) 
            ...
            if (Layout!=4) {
             switch(a) { 
                ...
                case 1:  ChrPtr = Str1; NameStr = STR1f; break;
                case 2:  ChrPtr = Str2; NameStr = STR2f; break;
                case 3:  ChrPtr = Str3; NameStr = STR3f; break;
                case 4:  ChrPtr = Str4; NameStr = STR4f; break;
                case 5:  ChrPtr = Str5; NameStr = STR5f; break;
                case 6:  ChrPtr = Str6; NameStr = STR6f; break; } }
             if (Layout==4) {
             switch(a) { 
                case 0:  ChrPtr = inputString; NameStr = "inputStr"; break;
                case 1:  ChrPtr = Ttr1; NameStr = TTR1f; break;
                case 2:  ChrPtr = Ttr2; NameStr = TTR2f; break;
                case 3:  ChrPtr = Ttr3; NameStr = TTR3f; break;
                case 4:  ChrPtr = Ttr4; NameStr = TTR4f; break;
                case 5:  ChrPtr = Ttr5; NameStr = TTR5f; break;
                case 6:  ChrPtr = Ttr6; NameStr = TTR6f; break; } } }  
           ...
           // => To set non-text macro on Keys S1/T1 to S/T6:
           // First set correct string lenth with any ASCII string then send numbers with RealTerm
           ////////////////////////////////////////////////////////////////////////////////////////
           if ((Found)&&(n>0)&&(inChar==0x0A)) { StrOK = true; ChrPtr[n-1] = '\0';}   // (inChar=='\n')
           if ((Found)&&(n>0)&&(b==0x00)) { ByteOK = true;}   
           } // while loop
           if ((Found)&&(StrOK)) {DoFileStrings(StrOK, NameStr, ChrPtr);  // Save the text/num strings 
                                  status(ChrPtr); }                       // Display the new string
           if ((Found)&&(ByteOK)) {status("Bytes Received"); for (i=0; i<n; i++) Serial.println(InBytes[i]); }
}
...
void buttonpress(int button)
{
  ...
  //////////////////////////////// reset if not [Cfg] + [End] [Home] [PageUp] [PageDown] key pressed
  if ((Layout==2) && (button!=8) && (button!=0) && (button!=2) && (button!=10) && (ConfigKeyCount>0)) 
     {ConfigKeyCount = 0; status(""); }       
  ...
  switch(button){
    ...
    case 7: //////////////////////////////////////////////// Volume Mute and change Layouts
      ...
      usb_hid.sendReport16(RID_CONSUMER_CONTROL, 0);
      break;
    case 8: // M4 S4 T4 ///////////////////////////////////////////// Send a text piece - could overide by sending serial text piece
    ...
    case 9: // M5 S5 T5 //////////////////////////////////////////// Open normal command prompt via Run window
      ...
      usb_hid.keyboardRelease(RID_KEYBOARD);                 delay(keydelay2);
      break;
    case 10: // M6 S6 T6 //////////////////////////////////////// M6 text or Enter key
  ...
  switch(button){
    ...
    case 5: // CONFIG //////////////////////////////////////////////////
      GetSysInfo(0); 
      status("End-Del Hme-Vo PgU-Vol PgD-L12"); 
...
void InitStr()
{ char *ChrPtr;
  if (!LittleFS.exists(STR1f)) strcpy(Str1,str1); else {ChrPtr = Str1; DoFileStrings(StrOK, STR1f, ChrPtr);}
  if (!LittleFS.exists(STR2f)) strcpy(Str2,str2); else {ChrPtr = Str2; DoFileStrings(StrOK, STR2f, ChrPtr);}
  if (!LittleFS.exists(STR3f)) strcpy(Str3,str3); else {ChrPtr = Str3; DoFileStrings(StrOK, STR3f, ChrPtr);}
  if (!LittleFS.exists(STR4f)) strcpy(Str4,str4); else {ChrPtr = Str4; DoFileStrings(StrOK, STR4f, ChrPtr);}
  if (!LittleFS.exists(STR5f)) strcpy(Str5,str5); else {ChrPtr = Str5; DoFileStrings(StrOK, STR5f, ChrPtr);}
  if (!LittleFS.exists(STR6f)) strcpy(Str6,str6); else {ChrPtr = Str6; DoFileStrings(StrOK, STR6f, ChrPtr);}

  if (!LittleFS.exists(TTR1f)) strcpy(Ttr1,str1); else {ChrPtr = Ttr1; DoFileStrings(StrOK, TTR1f, ChrPtr);}
  if (!LittleFS.exists(TTR2f)) strcpy(Ttr2,str2); else {ChrPtr = Ttr2; DoFileStrings(StrOK, TTR2f, ChrPtr);}
  if (!LittleFS.exists(TTR3f)) strcpy(Ttr3,str3); else {ChrPtr = Ttr3; DoFileStrings(StrOK, TTR3f, ChrPtr);}
  if (!LittleFS.exists(TTR4f)) strcpy(Ttr4,str4); else {ChrPtr = Ttr4; DoFileStrings(StrOK, TTR4f, ChrPtr);}
  if (!LittleFS.exists(TTR5f)) strcpy(Ttr5,str5); else {ChrPtr = Ttr5; DoFileStrings(StrOK, TTR5f, ChrPtr);}
  if (!LittleFS.exists(TTR6f)) strcpy(Ttr6,str6); else {ChrPtr = Ttr6; DoFileStrings(StrOK, TTR6f, ChrPtr);}
...
{  char *ChrPtr;
  ...
  Serial.println("VolumeMacroKeys version 127.0");

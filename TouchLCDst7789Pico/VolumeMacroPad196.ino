...
char DBArr[10][3] = {"00", "02", "03",  "06", "09", "12", "18", "25", "38", "50"};
char BBArr[10][3] = {"00", "03", "06",  "09", "12", "15", "18", "25", "38", "50"};
...
char DimArr[]      = "Dimmer Val              (%)";
char NormArr[]     = "LCD Bright              (%)";
  ...
  switch(button){
    ...
    case 3: // V+ or Delete or Escape(KeyBrd+Math)=Execute(Keybrd) //////////////////////////////////////
      ...
      if (Kbrd) {Kbrd = false;  status((char *)KBDisp);
                 ...
                 if (KeyBrdX<4) {KeyBrdByteNum = 0; KBDispPos = 0; SendBytesEnd();}  // Exit keybrd
                 ConfigButtons(1); break;}
...
void ConfigButtons(uint8_t rowcount) {  // rowcount=0 all 4 rows rowcount=2 last row only  rowcount=1 no background redraw 
  ...
  for (row = 0 + rowcount; row < rows; row++) {   // 3 rows 0,1,2 => b = 0+(0*4)=0 1+(0*4)=1 ... 0+(1*4)=4 ....
   ...
   for (row = 0; row < 5; row++) {PadB = padOutline[row+(b*5)]; PadC = padColor[row+(b*5)]; // row = 0,1,2 + (0 1 2 3 = Layout-1) * 3
        ...
        //                            308       36 + 0,1,2,3,4 * 38 = 38,74,112,150,188   10    20             
...
{ char *ChrPtr;
  ...
  ReadLCDBright(); b = (NormVal*100)/255;
...
{ unsigned long t, z;
  ...
  if ((KeyBrdByte[1]==0x64)&&(KeyBrdByte[2]==0x62))                    // *db*num = dim backlight pwm  *db*0,1-9
      {DimVal=dbArr[b];                                                // 
       ...
       DimArr[14] = DBArr[b][0]; DimArr[15] = DBArr[b][1]; 
  ...
  if ((KeyBrdByte[1]==0x62)&&(KeyBrdByte[2]==0x62))                    // *bb*num = normal backlight pwm  *bb*0,1-9 0 8/255 - 128/255 % PWM
      ...
      if (NormVal==0) digitalWrite(LCDBackLight, HIGH);                // Backlight Full ON
       ...
       NormArr[14] = BBArr[b][0]; NormArr[15] = BBArr[b][1];
...
//////////////////////////////////////////////////////////////////////
// From Keypad_240x320.ino in https://github.com/Bodmer/TFT_eSPI/
//////////////////////////////////////////////////////////////////////

...
int SpecialKey1 = 0;               // Select sequence of 3 keys pressed once
int SpecialKey2 = 0;               // Will then switch to second layout
...
uint16_t keydelay3 = 500;

// Create the screen object
TFT_eSPI tft = TFT_eSPI();

// Two sets of 12 button colours - use the colours defined in TFT_eSPI.h 
uint16_t Colours[2][12] =       {TFT_BLUE,      TFT_RED,       TFT_ORANGE,    TFT_DARKCYAN, 
...
uint16_t BackgroundColor[2] = {TFT_BLACK, TFT_BLACK};   // TFT_NAVY
...
const static char Str3[] = "cmd\r";                             // Str3[5] = {'c','m','d','\r','\0'};
const static char Str4[] = "winver\n";   
...
void loop() {
  ...
  if ((NowMillis - LastMillis) >= TimePeriod)  //test whether the period has elapsed
  {
    ...
    SpecialKey1 = SpecialKey2 = 0;     // Reset Special keys for Alt Menu
  }
...
void buttonpress(int button)
{
  ...
  switch(button){
    case 0: ///////////////////////////////////////////////// Cut = control + x                             
      ...
      has_key = true;
      delay(keydelay); 
      if (SpecialKey2==0) SpecialKey1 = 1;                  // First key pressed to change layout button + 1;  
      break;
    case 1: ///////////////////////////////////////////////// Copy = control + c
      ...
      has_key = true;
      delay(keydelay);
      if (SpecialKey1==1)SpecialKey2 = 2; // button + 1;     // Second key pressed to change layout
      break;
    case 2: ////////////////////////////////////////////////// Paste = control + v
      if ((SpecialKey1==1)&&(SpecialKey2==2)) DoKeyboard1(); // Last key to change layout - do before key 
      keycode[0] = CtrL;
      keycode[1] = HID_KEY_V;
      usb_hid.keyboardReport(RID_KEYBOARD, 0, keycode);
      has_key = true;
    ...
    case 3: /////////////////////////////////////////////////// Volume Increase
      ...
      delay(keydelay);
      break;
    case 4: /////////////////////////////////////////////////// Alt + PrintScreen - focus window - some mini keyboards cannot do this
      keycode[0] = AltL; // or use HID_KEY_GUI_LEFT do not use KEYBOARD_MODIFIER_LEFTGUI
      keycode[1] = PrtScr;
      keycode[2] = 0x00; // 
      usb_hid.keyboardReport(RID_KEYBOARD, 0, keycode);
      delay(keydelay2);
    ...
    case 5: /////////////////////////////////////////////////// Open admin powershell including UAC completion
      ...
      usb_hid.keyboardRelease(RID_KEYBOARD);
      delay(keydelay2);
      usb_hid.keyboardPress(RID_KEYBOARD, 'A'); // A admin powershell i normal powershell g Computer Management
      delay(keydelay2);
      usb_hid.keyboardRelease(RID_KEYBOARD);
      delay(keydelay3);
      keycode[0] = ArrLeft;
      ...
      delay(keydelay2);
      break;
    case 6: ///////////////////////////////////////////////// Open Run window with last coommand visible
    ...
    case 8: /////////////////////////////////////////////// Send a long text piece
      for (n=0;  n<strlen(Str2); n++) 
          ...
          delay(keydelay2); }
      break;
    case 9: ////////////////////////////////////////////// Open normal command prompt via Run window
      ...
      usb_hid.keyboardRelease(RID_KEYBOARD);
      delay(keydelay2);
      for (n=0;  n<strlen(Str3); n++)                     // Str3:Cmd Prompt Str4: winver
          {usb_hid.keyboardPress(RID_KEYBOARD, Str3[n]);
          delay(keydelay2);
          usb_hid.keyboardRelease(RID_KEYBOARD);
          delay(keydelay2); }
      break;
...
void hid_report_callback(uint8_t report_id, hid_report_type_t report_type, uint8_t const* buffer, uint16_t bufsize)
{
  ...
  **********************************************************/
  ...
  if (CapsLock!=CapsLockPrev){       Change = true;
  if (CapsLock)                    { Colours[0][8] = TFT_DARKGREEN;  // Use unique colour       // TFT_OLIVE TFT_DARKGREEN TFT_MAROON
                                     Colours[1][8] = TFT_DARKGREEN;  // Use unique colour
                                     ConfigButtons(); }
  if (!CapsLock)                   { Colours[0][8] = TFT_LIGHTGREY;  // Use a unique colour
                                     Colours[1][8] = TFT_BLUE;       // Use a unique colour
                                     ConfigButtons(); }  }           // Draw Buttons and Labels
                                     
  if (NumLock!=NumLockPrev){         Change = true;                                 
  if (NumLock)                     { Colours[0][9] = TFT_DARKGREEN;  // Use unique colour
                                     Colours[1][9] = TFT_DARKGREEN;  // Use unique colour
                                     ConfigButtons(); }
  if (!NumLock)                    { Colours[0][9] = TFT_LIGHTGREY;  // Use a unique colour
                                     Colours[1][9] = TFT_LIGHTGREY;  // Use a unique colour
                                     ConfigButtons(); }  }           // Draw Buttons and Labels
                                    
  if (ScrollLock!=ScrollLockPrev){   Change = true;                                     
  if (ScrollLock)                  { Colours[0][10] = TFT_DARKGREEN;  // Use unique colour  // TFT_GREENYELLOW almost same
                                     Colours[1][10] = TFT_DARKGREEN;  // Use unique colour
                                     ConfigButtons(); }
   if (!ScrollLock)                { Colours[0][10] = TFT_LIGHTGREY;  // Use a unique colour
                                     Colours[1][10] = TFT_RED;        // Use a unique colour
                                     ConfigButtons();  } }            // Draw Buttons and Labels  

...
// Set theory: not-subset-or-equal disjoint-union union intersection element-of not-element-of empty-set subset-of not-subset-of set-difference 
//             subset-or-equal plus-sign-above-equals-sign double-square-intersection double-square-union intersection-with-dot amalgamation join
...
//              sup-0 sup-1 sup-2 sup-n sup-i sup-j sub-0 sub-1 sub-2 sub-n sub-i sub-j
...
{ if (Restore) { if (Pbutton==3) {VolDisable = VolDisablePrev; MuteDisable = MuteDisablePrev;} }                              
                 ...
                 if (Pbutton==1) { PadKeyR = Math;      PadKeyG = Media; Kbrd = Media = NumKeys = MouseKeys = false;}
                 if (Pbutton==2) { PadKeyB = Kbrd;      PadKeyG = Media; Math = Media = NumKeys = MouseKeys = false;}
                 if (Pbutton==3) { PadKeyG = Media;                      Math = Kbrd  = NumKeys = MouseKeys = false;}                   
                 if (Pbutton==4) { PadKeyS = NumKeys;   PadKeyG = Media; Math = Media = Kbrd    = MouseKeys = false;}
                 if (Pbutton==5) { PadKeyY = MouseKeys; PadKeyG = Media; Math = Media = NumKeys = Kbrd      = false;}  
               }
  ConfigButtons(1);
  status(" ");
}

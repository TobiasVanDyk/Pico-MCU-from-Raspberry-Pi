...
int BlackKeyCount = 1;             // Special Operation 
///////////////////////////////////////////////////////////////////////////////
...
void setup() {
  ...
  */                                 // **[4]**
  ...
  BlackKeyCount = 1;            // Special Operation
...
void loop() {
  ...
  if ((NowMillis - LastMillis) >= TimePeriod) //test whether the period has elapsed
     {digitalWrite(LCDBackLight, LOW);        // Backlight Off
      ...
      VolMuteCount = 0;                       // VolMuteKey pressed Count = 0 
      BlackKeyCount = 0;                      // [i][End] sequence will delete files
...
void buttonpress(int button)
{
  ...
  if ((Layout==2) && (button!=8) && (BlackKeyCount>0)) {BlackKeyCount = 0; // reset if not [i] + [End] key pressed
                                                        status(""); }
  ...
  switch(button){
    ...
    case 5: 
      GetSysInfo(); 
      status("Press END key to delete all files"); 
      if (BlackKeyCount==0) BlackKeyCount++ ; else BlackKeyCount = 0;
      break;
    case 6:
      keycode[0] = ArrRight;
      usb_hid.keyboardReport(RID_KEYBOARD, 0, keycode); delay(keydelay); 
    ...
    case 7:
      ...
      break;
    case 8:
      if (BlackKeyCount==1) {status("Files deleted"); BlackKeyCount-- ; DeleteFiles(); break; }
...
{ //while (!Serial) { delay(100); }
  ...
  Serial.println("VolumeMacroKeys version 121.0");
